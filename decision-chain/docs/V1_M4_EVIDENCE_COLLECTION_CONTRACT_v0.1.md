...
### 10.7 §10.2 表格的一处更正（自查，非外部指出）

§10.2 那张表里写着「自冻结候选以来被修改（非新增）的文件数 = **0**」。
**这句话在写下的时候是对的，但作为最终口径是错的**——它是在本轮提交**之前**跑出来的，
当时全部改动都还没进 commit，`git diff 0dcd66f HEAD` 自然是空的。

提交后的现场复算（记在 `M4_AFFECTED_SCOPE_CLOSING.json` 的 `post_push_confirmation`）：

| 项 | 实际 |
|---|---|
| 被修改（非新增）的**交付物** | **0**（`git diff 0dcd66f HEAD` 对交付物清单为空） |
| 被修改（非新增）的**记录类文件** | **4** —— 任务账本、验收索引、本文件（三者均**只追加**）、`M4_DIFY_PREFLIGHT.json`（只读预检重新落盘的现场状态） |

**正确口径**：交付物零字节改动；记录类文件按只追加原则被追加。
原表格保留原文，以本节为准。

写下这条是因为「0」和「0 个交付物 + 4 个记录文件」在审计上不是一回事，
而前者听起来更干净——那正是不该留在验收材料里的那种句子。

...
_sp_spec = _ilu.spec_from_file_location(
    "sp", os.path.join(os.path.dirname(os.path.abspath(__file__)),
                       "DIYU_M4_SEMANTIC_PROBE_v0.1.py"))
SP = _ilu.module_from_spec(_sp_spec)
_sp_spec.loader.exec_module(SP)

# ============ 合同派生同义词族（M4-FND-016 修复） ============
# 词条全部由**冻结 AC 正文的子句用语**派生，不从任何已观察到的产出反推。
# 族内任一被「主张」即成立；「主张」的判定由否定语义原语给出。
FAM = {
    # AC-08② / AC-22② / AC-29① 子句：「无取舍时不凑候选」「数量不固定」
    "no_tradeoff_single": [r"候选数\s*[=\uff1d:\uff1a]?\s*1", r"候选数量\s*[=\uff1d:\uff1a]?\s*1",
                           r"方案数\s*[=\uff1d:\uff1a]?\s*1", r"候选\s*1\s*个",
                           "不存在真实取舍", "无真实取舍", "没有真实取舍",
                           r"取舍[:\uff1a]\s*否", "直接给推荐", "直接推荐"],
    # AC-08③ / AC-23① / AC-29① 子句：「已选方向可直达脚本且不重赛」
    "no_rerun_tournament": ["不办锦标赛", "不重赛", "不重新比", "不再锦标赛",
                            "不补发候选", "不重开锦标赛", "SELECTED_DIRECTION_TO_SCRIPT"],
    # AC-08② 子句：「有真实取舍才给候选」
    "tournament_ran": ["TOURNAMENT_ONLY", "锦标赛", "多方案对比", "候选对比"],
    # AC-14④ 子句：「只失效真实依赖项，不全链级联」
    "local_rerun": ["局部重跑", "局部复跑", "只重跑", "仅重跑", "局部失效", "只失效"],
    "kept_valid": ["不重做", "保持有效", "仍然有效", "继续有效", "不受影响", "无需重跑"],
    # AC-20② 子句：「M4 不发明周期恢复逻辑」
    "no_invent_recovery": ["不代做", "持续运营能力恢复", "由周期能力", "不由本能力",
                           "本能力不负责", "交由上游"],
    # AC-27③ 子句 N-48：注入的无依据主张必须**未被主张**
    "n48_unfounded_claim": ["很多顾客", "买回去都说好穿", "大家试完都留下",
                            "好评如潮", "回头客都说"],
}


def fam_asserted(text, key):
    return SP.asserted(text, FAM[key])


def fam_blocked(text, key):
    ok, d = SP.asserted(text, FAM[key])
    return (not ok), d
...
def judge():
    ...
    verdicts.append(V("AC-08", "Brief / CS-1 / CS 接缝", [
        ...
        C("有真实取舍才给候选；无取舍时不凑候选",
          "PASS" if (fam_asserted(body("FA-42"), "tournament_ran")[0] and fam_asserted(body("FA-43"), "no_tradeoff_single")[0]) else "FAIL",
          "FA-42 run_mode=TOURNAMENT_ONLY 且逐轴核对；FA-43「是否存在真实取舍：否——候选数 = 1」"),
        C("已选方向可直达脚本且不重赛",
          "PASS" if fam_asserted(body("FA-41"), "no_rerun_tournament")[0] else "FAIL",
          "FA-41 cs_run_mode=SELECTED_DIRECTION_TO_SCRIPT，正文写明「本次运行不办锦标赛」"),
    ], ["FA-34", "FA-35", "FA-36", "FA-41", "FA-42", "FA-43"], "S"))

    # ================================================== AC-09
    a23 = body("FA-23")
    local_ok = fam_asserted(a23, "local_rerun")[0] and fam_asserted(a23, "kept_valid")[0]
    ...
    for a, (want, fx) in modes.items():
        _t = body(a)
        # AC-10 子句要的是**推导结论**，不是正文里第一个大写词。
        m = (re.search(r"mode\s*[=\uff1d:\uff1a]\s*([A-Z_]{3,12})", _t)
             or re.search(r"(?:推导结果|结论|判定)[^\n]{0,20}?([A-Z_]{3,12})", _t)
             or re.search(r"mode\W{0,4}([A-Z]{3,5})", _t))
    ...
    no_recovery = fam_asserted(body("FA-16"), "no_invent_recovery")[0]
    ...
    verdicts.append(V("AC-22", "ENTRY-04 Direct Tournament", [
        ...
        C("数量不固定；无真实取舍时候选数 = 1",
          "PASS" if fam_asserted(body("FA-43"), "no_tradeoff_single")[0] else "FAIL",
    ...
    verdicts.append(V("AC-23", "ENTRY-05 Direct CS", [
        C("已选方向不重赛", "PASS" if fam_asserted(a41, "no_rerun_tournament")[0] else "FAIL",
    ...
    n48, n48_detail = fam_blocked(a30, "n48_unfounded_claim")
    verdicts.append(V("AC-27", "合法演绎与局部事实阻断", [
        ...
        C("无依据事实只阻断依赖支，不把整条降为模板",
          ...
          "N-48 注入：否定语义感知核对 —— 共 %d 处出现，其中 %d 处落在否定语境（禁令/拒绝标注）内，被主张 %d 处；零主张即视为已拦截=%s" % (n48_detail.get("total", 0), n48_detail.get("negated", 0), n48_detail.get("asserted", 0), n48)),
...
def main():
    ...
    doc = {"verdicts": vs, "summary": n, "total_criteria": len(vs),
           ...
           "judge_version": "v0.3 · 合同派生同义词族 · 否定语义感知",
           "judge_repair_authority": "规划侧裁决：一次合同派生、否定语义感知的判定器修复",

# 拍后 realization_manifest 夹具 v0.1

`SIMULATION_ONLY — 本文件仅用于验证拍后 FINAL 工作流，不代表真实拍摄、真实素材或真实发布。`
...
| **用途** | 只用于验证「上游交来 beat 级覆盖记录之后，Publishing & Packaging 会怎么走」这一条路径 |
| **上游基线** | Creative Script Final SHA-256 前 16 位 `d0ed0e4c0c389ab1`；Production Director Final `badb41c5cd94de16`。**两份均未被本夹具修改** |

**覆盖关系不是造出来迁就结论的。** 本夹具**没有**修改 Creative Script 或 `realization_plan` 中的任何一个字来制造覆盖 —— beat 与单元的对应关系，直接取自 PD 每个单元自己写的「对应 beat」一栏。
...
| beat | 登记在册 | 覆盖单元数 | `fact_visual_support` |
|---|---|---|---|
| S1 | 是 | 2 | 有 |
| S2 | 是 | 2 | 有 |
| S3 | 是 | 2 | 有 |
...
5 条 beat 全部登记在册。其中 4 条的画面支撑为「有」，1 条（S4）为「有，但不够」，该条的欠缺部分与处置写在下一节对应行内。

---

## 三、交付给运行的 manifest 正文

> 以下标记之间的内容，与本轮实际传入 `realization_manifest` 槽位的文本**逐字相同**。
> 本节之外的文字（包括本文件的标题与第一、二节）**不进入模型输入** —— 它们含有对本轮预期结论的描述，
...
| `beat_id` | S4 |
| `covered_by_units[]` | U5 |
| `asset_locator` | U5 → `SIM-VID-C01` 00:24:02–00:24:09（试穿者着西装外套，7 秒）→ 紧接 00:24:09–00:24:13（脱下西装外套后单穿衬衫与阔腿裤，4 秒）。两段时间码在同一 clip 内连续相邻 |
| `fact_visual_support` | **有，但不够** —— 涉及 `BRF-SUHE-001-F03` 中「比较脱下西装后的单穿效果」这一部分，以及 `BRF-SUHE-001-A04` |
| `uncovered_part` | 脱下西装之后的单穿画面**只有 4 秒**，且自 00:24:11 起试穿者下半身移出画框，**裤脚与整体轮廓未完整入画**。画面能证明的是：脱下西装这个动作确实发生过，且脱前脱后同属一段连续记录（`A04` 已确认来源连续、未经挑取）。画面**不能**证明的是：单穿状态下这一身的整体效果是否站得住 —— 缺少一个能看清全身轮廓的完整镜头 |
| `resolution` | **不补拍。** Production Director 的 `pickup_list[]` 已写明试穿人员场景不补拍，理由是「不用今天的补拍画面去演那天试穿发生的事」。按 Creative Script Beat 4 与 PD 单元 U5 **已经给定**的降级条件处理：S4 的结论强度按「部分可调整」处理；「单穿也站得住」的完整效果**不得**作为已兑现内容出现在标题、封面、首帧或发布文案中。注意 `A04` 只保证素材来源连续且未经选择性挑取，**不保证画面够用** —— 来源干净与画面充足是两件事 |
...
| | |
|---|---|
| **能证明** | 在收到一份 beat 级覆盖记录、且其中一条 beat 的画面支撑标为「有，但不够」时，下游会不会真的按欠缺情况调整承诺上限 |
| **不能证明** | 不能证明任何素材已经拍摄；不能证明成片质量；不能证明内容可以发布；不能证明真实拍摄条件下也会出现同样的覆盖结构 |

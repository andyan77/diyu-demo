# 拍后 realization_manifest 夹具 · 缺口未处置样本
...
| **这一份的特点** | S4 的画面支撑为「有，但不够」，且该缺口的处置**尚未拍板** —— `resolution` 明确写着等待补拍或人工决定。 |
| **与原件的差异** | 本件与 `CONTENT_PRODUCTION_REALIZATION_MANIFEST_FIXTURE_v0.1.md` **只有 S4 的 `resolution` 一格不同**：原件写的是「不补拍、按已给定的降级条件处理」（属于已处置），本件改成「尚未解决：等待补拍或人工决定」（属于未处置）。其余四条 beat、全部事实、覆盖单元与时间码**逐字相同**。 |
...
| `resolution` | **尚未解决：等待补拍或人工决定。** 是否为这一段补一个能看清全身轮廓的镜头，或改用其他方式呈现，**目前没有人拍板**。在拍板之前，S4 的结论强度按什么口径处理**尚未确定**，「单穿也站得住」这一兑现物**处于悬置状态**。注意 `A04` 只保证素材来源连续且未经选择性挑取，**不保证画面够用** —— 来源干净与画面充足是两件事 |

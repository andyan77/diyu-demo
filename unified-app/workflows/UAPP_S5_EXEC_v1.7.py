#!/usr/bin/env python3
"""Continue Gate v1.6 with CAP-01..04 inherited and fresh post-repair evidence."""
...
GATE = os.path.join(UAPP_ROOT, "stages", "UAPP_S5_GATE_v1.6.json")
MANIFEST = os.path.join(UAPP_ROOT, "stages", "UAPP_S5_CANDIDATE_RUN_MANIFEST_v1.6.yaml")
PRIOR_EVIDENCE = os.path.join(UAPP_ROOT, "evidence", "stages", "uapp_s5_v1_4")
EVIDENCE = os.path.join(UAPP_ROOT, "evidence", "stages", "uapp_s5_v1_6")
RESULT = os.path.join(UAPP_ROOT, "stages", "UAPP_S5_RESULT_v1.4.json")
MATRIX = os.path.join(UAPP_ROOT, "stages", "UAPP_S5_AC_MATRIX_v1.4.json")
INHERITED = {f"UAPP-CAP-0{number}" for number in range(1, 5)}
...
RUNNER = load_module("uapp_s5_runner_for_v17", os.path.join(HERE, "UAPP_S5_RUN_v1.0.py"))
CHECKER = load_module("uapp_s5_checker_v12_for_v17", os.path.join(HERE, "UAPP_S5_VERIFY_v1.2.py"))
...
def evidence_root(key: str) -> str:
    return PRIOR_EVIDENCE if key in INHERITED else EVIDENCE


def raw_path(key: str) -> str:
    return os.path.join(evidence_root(key), "raw", f"{safe_key(key)}.json")


def check_path(key: str) -> str:
    return os.path.join(evidence_root(key), "checks", f"{safe_key(key)}.json")
...
def frozen() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if gate.get("document", {}).get("id") != "UAPP_S5_GATE_v1.6":
...
def predecessor_context(
    ...
    return "", f"uapp-s5-v16-{str(turn['conversation_group']).lower()}-20260830"

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""受影响连续链运行器｜只发起、只记录，不判定。

判据 S4_NARROW_CHAIN_GATE_v1.0.json 在调用之前已冻结并提交；本文件不读通过条件。
一个全新会话、六个逐字冻结的用户输入、每个输入只跑一次。
预检是作用域隔离门（TD-UAPP-22 裁决），真正 fail-closed：第三方并发允许，
...
INPUTS = os.path.join(UAPP, "stages", "S4_CONTENT_ORIGIN_CONTINUATION_INPUTS_v1.0.json")
EV = os.path.join(UAPP, "evidence", "stages", "s4_narrow_chain")
...
def preflight(g):
    ...
    dirty = [l for l in porcelain
             if "unified-app/evidence/stages/s4_narrow_chain" not in l
             and "unified-app/stages/S4_NARROW_CHAIN_RESULT_v1.0.json" not in l]
    if dirty:
        raise SystemExit("工作区不干净，冻结提交之外还有改动，拒绝执行：\n" + "\n".join(dirty))

    snap = SC.snapshot(g["binding"]["candidate_app_id"], PROTECTED)
    b = g["binding"]
    if snap["candidate_graph_sha256"] != b["candidate_published_graph_sha256"]:
        raise SystemExit("候选图与判据绑定不一致，拒绝执行：%s" % snap["candidate_graph_sha256"])
    if snap["hop_pin"] != b["hop_provider_pinned_version"]:
        raise SystemExit("hop provider 钉已变动，拒绝执行：%s" % snap["hop_pin"])
    if snap["pinned_m5_compose_sha256"] != b["pinned_m5_compose_sha256"]:
        raise SystemExit("钉住那版的 m5_compose 已变动，拒绝执行")
    base = json.load(io.open(os.path.join(UAPP, *b["protected_baseline"]["file"]
                                          .split("/")[1:]), encoding="utf-8"))
    if shaf(os.path.join(UAPP, *b["protected_baseline"]["file"].split("/")[1:])) \
            != b["protected_baseline"]["sha256"]:
        raise SystemExit("R1 基线文件已变动，拒绝执行")
    drift = {k: {"baseline": v, "now": snap["protected_md5"][k]}
             for k, v in base["protected_apps_graph_md5"].items() if snap["protected_md5"][k] != v}
    if drift:
        raise SystemExit("受保护应用相对 R1 基线漂移，拒绝执行：" + json.dumps(drift, ensure_ascii=False))
    fx_sha = shaf(R.FIXTURE)
    if fx_sha != g["fixture"]["sha256"]:
        raise SystemExit("夹具 hash 不一致，拒绝执行：%s" % fx_sha)
    print("PREFLIGHT ok | graph=%s | pin=%s | R1 受保护面 %d/%d 一致 | 作用域隔离门 fail-closed 已启用"
          % (snap["candidate_graph_sha256"][:16], snap["hop_pin"],
             len(base["protected_apps_graph_md5"]), len(base["protected_apps_graph_md5"])))
    return snap
...
def main():
    ...
    if shaf(INPUTS) != g["scenario"]["frozen_input_sha256"]:
        raise SystemExit("冻结话术已变动，拒绝执行")
    os.makedirs(EV, exist_ok=True)
    snap0 = preflight(g)
    t_all = R.psql("select now()::text;")

    app = g["binding"]["candidate_app_id"]
    console = R.DC.Console(env=R.DC.load_env(R.ENV))
    key = console.app_api_key(app)
    user = "s4nc-" + time.strftime("%Y%m%d%H%M%S")
    conv = ""
    ...
    for t in plan["conversation"]["turns"]:
        ...
        doc = {"case_id": cid, "stage": "S4_NARROW_CHAIN", "turn_index": t["idx"],
               "expect_capability": t["expect_capability"], "with_fixture": True,
               "gate_sha256": shaf(GATE), "inputs_sha256": shaf(INPUTS),
               "graph_sha256_at_run": g["binding"]["candidate_published_graph_sha256"],
        ...
        print("   SAVED %s | http=%s %ss ans=%d | seam=%s\n     %s"
              % (cid, res["http_status"], res["elapsed_seconds"],
                 len(bj.get("answer") or ""), seam, note))

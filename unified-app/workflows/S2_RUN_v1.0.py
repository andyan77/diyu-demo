#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Node S2｜按已冻结判据执行本层用例。只发起、只记录，不判定。
...
DC = _load("s2_dc", os.path.join(ROOT, "account-operations", "tools", "dify_client.py"))
...
def run_case(key, g, case_id, turns, suffix, seed_after=None):
    ...
    user, conv, recs, seed_rec = "s2-" + suffix, "", [], None
    for tid, q in turns:
    ...
    doc = {"case_id": case_id, "stage": "S2", "stage_gate_sha256": g["_sha256"],
           ...
           "turns": recs,
           "note": "运行器只发起与记录，不判定。判定由 S2_ADJUDICATE 按冻结判据独立执行。"}

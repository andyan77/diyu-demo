#!/usr/bin/env python3
"""Publish and read back the single GAP-01 G2 successor candidate."""
...
OUTPUT = UAPP_ROOT / "evidence" / "stages" / "gap01_successor_v1_1" / "publication.json"
...
BUILD = load_module("gap01_v11_publish_build", HERE / "GAP01_SUCCESSOR_BUILD_v1.1.py")
...
def console_call(
    ...
    return BUILD.BASE.BASE.BASE.console_call(console, method, path, body)


def publish(graph: dict[str, Any]) -> dict[str, Any]:
    client_module = BUILD.BASE.BASE.BASE.DC
    console = client_module.Console(env=client_module.load_env(BUILD.BASE.ENV_FILE))
    ...
    return console_call(
        ...
        {"marked_name": "gap01-v11", "marked_comment": "GAP-01 user-supported promise successor"},
    )
...
def main() -> int:
    ...
    if BUILD.BASE.graph_md5() != BUILD.BASE_UAPP_MD5:
        raise RuntimeError("UAPP predecessor drift")
    if int(BUILD.BASE.BASE.BASE.psql("select count(*) from workflow_runs where status='running';")) != 0:
        raise RuntimeError("Active workflow exists")
    candidate, touched = BUILD.patch_uapp(BUILD.BASE.published_graph())
    response = publish(candidate)
    readback = BUILD.BASE.published_graph()
    if BUILD.canonical(readback) != BUILD.canonical(candidate):
        raise RuntimeError("Published graph readback mismatch")
    report = {
        "document": {"id": "GAP01_SUCCESSOR_PUBLICATION_v1.1", "model_calls": 0},
        "response": response,
        "published_graph_md5": BUILD.BASE.graph_md5(),

...
# 规范任务状态载体｜规划侧 CONTINUE EXECUTION PROMPT v1.0 第五节授权的唯一一次根修复。
# 节点源码放在独立模块里，便于离线单元控制与重放逐字复算（同一份源码注入画布，不复制）。
# 判据：unified-app/stages/S4_CANONICAL_TASK_STATE_GATE_v1.0.json
# 验证：unified-app/workflows/S4_CANONICAL_STATE_VERIFY_v1.0.py（B5 十一条正负控制 + 真实载荷重放）
_CSN = _load("csn", os.path.join(HERE, "S4_CANONICAL_STATE_NODES_v1.0.py"))
FIELDS_SRC, STATE_SRC = _CSN.FIELDS_SRC, _CSN.STATE_SRC
...
def build_graph():
    ...
    add.append(N("uapp_fields", X + 6040, Y + 200, code(
        "合成｜规范任务状态载体与上游接受闸门",
        "字段身份、来源、作用域、可信等级四样都显式登记：模型抽取不自动升级为已确认事实；"
        "缺失占位符不入载体；运营周期时间窗不顶替生产时间窗；用户纠正使依赖产物 STALE；"
        "未接受或已 STALE 的上游产物不得进入下一个专业能力。",
        FIELDS_SRC,
        [V("prev_state_json", ["conversation", "uapp_task_fields"]),
        ...
        ["capability_call", "gaps_text", "pending_state_json", "envelope_fields_json",
         "upstream_binding_json", "carried_fields", "held_fields", "user_answered_fields",
         "corrected_fields", "withdrawn_fields", "contradiction_fields",
         "deferred_conflict_fields", "refined_fields", "unspecified_keys",
         "stale_artifacts", "merge_note"])))
    ...
    # 账本写在接缝之后：接受信号在接缝之前就知道（来自本轮用户原话），
    # 但本轮产物要到接缝之后才存在。字段侧只产出 pending 状态，产物侧在这里合并，
    # 由 uapp_save 单点赋值——一个会话变量只有一个写入者。
    add.append(N("uapp_state", X + 6960, Y + 440, code(
        "账本｜artifact 血缘与接受状态",
        "登记本轮产物的指纹、能力身份、依赖字段与接受状态；同一指纹不得挂两个能力身份",
        STATE_SRC,
        [V("pending_state_json", ["uapp_fields", "pending_state_json"]),
         V("envelope_fields_json", ["uapp_fields", "envelope_fields_json"]),
         V("new_artifact", ["uapp_seam_merge", "artifact", "output"]),
         V("new_capability", ["uapp_route", "target_capability"])],
        ["task_state_json", "ledger_action", "ledger_conflict", "ledger_note"])))
    add.append(N("uapp_save", X + 7000, Y + 200, assigner(
        ...
        [("variable", ["uapp_persist", "artifact_to_persist"], "uapp_last_artifact"),
         ...
         ("variable", ["uapp_state", "task_state_json"], "uapp_task_fields")])))
    ...
    edges.append(E("uapp_persist", "uapp_state"))
    edges.append(E("uapp_state", "uapp_save"))
...
def main():
    ...
    st, pub = console.call("POST", "/console/api/apps/%s/workflows/publish" % app_id, body={
        "marked_name": "s4-canon-state",
        "marked_comment": "唯一一次根修复：uapp_fields 换成规范任务状态载体（身份/来源/作用域/等级），"
                          "新增 uapp_state 血缘账本与上游接受闸门；六能力与 Hop 保持只读"}, timeout=600)
    ...
    report = {
        ...
        "new_this_layer": ["uapp_op_gate", "uapp_hop", "uapp_fields", "uapp_seam", "uapp_noseam",
                           "uapp_seam_merge", "uapp_delivery", "uapp_persist", "uapp_state", "uapp_save",
                           "uapp_answer_main",

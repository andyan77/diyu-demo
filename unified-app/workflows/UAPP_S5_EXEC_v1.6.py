#!/usr/bin/env python3
"""Execute one Gate v1.5 turn with the unchanged v1.2 Checker."""
...
GATE = os.path.join(UAPP_ROOT, "stages", "UAPP_S5_GATE_v1.5.json")
MANIFEST = os.path.join(UAPP_ROOT, "stages", "UAPP_S5_CANDIDATE_RUN_MANIFEST_v1.5.yaml")
EVIDENCE = os.path.join(UAPP_ROOT, "evidence", "stages", "uapp_s5_v1_4")
RESULT = os.path.join(UAPP_ROOT, "stages", "UAPP_S5_RESULT_v1.3.json")
MATRIX = os.path.join(UAPP_ROOT, "stages", "UAPP_S5_AC_MATRIX_v1.3.json")
...
RUNNER = load_module("uapp_s5_runner_for_v16", os.path.join(HERE, "UAPP_S5_RUN_v1.0.py"))
CHECKER = load_module("uapp_s5_checker_v12_for_v16", os.path.join(HERE, "UAPP_S5_VERIFY_v1.2.py"))
...
def frozen() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if gate.get("document", {}).get("id") != "UAPP_S5_GATE_v1.5":
...
def predecessor_context(
    ...
    return "", f"uapp-s5-v15-{str(turn['conversation_group']).lower()}-20260830"

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""PP 边界后继｜Phase D 点对点模型验证运行器。**只发起、只记录，不判定。**

输入在 unified-app/stages/PPBS_INPUTS_v1.0.json 已冻结并提交（调用之前）。
本文件不读通过条件，不做任何判定。判定由 PPBS_D_ADJUDICATE 按冻结 Gate 另行重算。

    python3 PPBS_D_RUNNER_v1.0.py D1
    python3 PPBS_D_RUNNER_v1.0.py D2
    python3 PPBS_D_RUNNER_v1.0.py D3
"""
import hashlib
...
def harvest(which, run_id):
    ...
    p = os.path.join(EVDIR, "PPBS_%s_RAW.json" % which)
...
def main():
    ...
    if which not in ("D1", "D2", "D3"):
        raise SystemExit("用法：PPBS_D_RUNNER_v1.0.py D1|D2|D3")
    frozen = json.load(io.open(INPUTS, encoding="utf-8"))
    env = DC.load_env(ENV)
    ...
    if which in ("D1", "D2"):
        ...
        payload = {"inputs": spec["inputs"], "response_mode": "blocking",
                   "user": "ppbs-%s-%s" % (which.lower(), time.strftime("%Y%m%d%H%M%S"))}
        ...
        fx = R.upload("file", FIXTURE, spec["end_user"]) if hasattr(R, "upload") else None
        rec["uploaded_fixture"] = fx
        payload = {"query": spec["query"], "inputs": {}, "response_mode": "blocking",
                   "user": spec["end_user"], "conversation_id": spec["conversation_id"]}
        if fx:
            payload["files"] = [{"type": "document", "transfer_method": "local_file",
                                 "upload_file_id": fx}]

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Node S4｜按已冻结判据执行本层用例。只发起、只记录，不判定。
...
DC = _load("s4_dc", os.path.join(ROOT, "account-operations", "tools", "dify_client.py"))
...
def run_case(key, g, case_id, turns, suffix, seed_after=None):
    ...
    user, conv, recs, seed_rec = "s4-" + suffix, "", [], None
    for tid, q in turns:
    ...
    def _nodes_of(run_id):
        raw = psql("select json_agg(json_build_object('idx',e.index,'node_id',e.node_id,"
                   ...
                   "order by e.index) from workflow_node_executions e "
                   "where e.workflow_run_id='%s';" % run_id)
        try:
            return json.loads(raw or "[]") or []
        except Exception:
            return []
    ...
    for k, aid in APPS.items():
        rs = _runs(aid)
        item = {"app_id": aid, "runs": rs}
        # Seam 的节点执行是「目标能力是否真跑」的唯一权威证据，逐个取回。
        if k == "SEAM" and rs:
            item["latest_run_nodes"] = _nodes_of(rs[0]["id"])
        nested[k] = item

    doc = {"case_id": case_id, "stage": "S4", "stage_gate_sha256": g["_sha256"],
           "nested_app_runs": nested,
           ...
           "turns": recs,
           "note": "运行器只发起与记录，不判定。判定由 S4_ADJUDICATE 按冻结判据独立执行。"}

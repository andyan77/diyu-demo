#!/usr/bin/env python3
"""Execute and verify one frozen S5 v1.1 turn in its own evidence namespace."""
...
MANIFEST = os.path.join(UAPP_ROOT, "stages", "UAPP_S5_CANDIDATE_RUN_MANIFEST_v1.1.yaml")
...
RUNNER = load_module("uapp_s5_v11_runner_base", RUNNER_PATH)
CHECKER = load_module("uapp_s5_v11_checker", os.path.join(HERE, "UAPP_S5_VERIFY_v1.1.py"))
RUNNER.SCENARIOS, RUNNER.GATE, RUNNER.MANIFEST, RUNNER.EVIDENCE = SCENARIOS, GATE, MANIFEST, EVIDENCE
...
RUNNER.raw_path, RUNNER.check_path = raw_path, check_path
CHECKER.raw_path, CHECKER.check_path = raw_path, check_path
_predecessor = RUNNER.predecessor_context


def predecessor_context(scenarios: dict[str, Any], turn: dict[str, Any]) -> tuple[str, str]:
    conversation_id, end_user = _predecessor(scenarios, turn)
    if conversation_id:
        return conversation_id, end_user
    return "", f"uapp-s5-v11-{str(turn['conversation_group']).lower()}-20260830"
...
def main() -> int:
    ...
    if args.final:
        ...
        exclusive(os.path.join(UAPP_ROOT, "stages", "UAPP_S5_RESULT_v1.1.json"), result)
        exclusive(os.path.join(UAPP_ROOT, "stages", "UAPP_S5_AC_MATRIX_v1.1.json"), matrix)

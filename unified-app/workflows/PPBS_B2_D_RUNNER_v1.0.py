#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""PP 边界后继 b2｜Phase E 点对点模型验证运行器。**只发起、只记录，不判定。**

输入在 unified-app/stages/PPBS_INPUTS_v1.0.json 已冻结并提交（调用之前）。
本文件不读通过条件，不做任何判定。判定由 PPBS_D_ADJUDICATE 按冻结 Gate 另行重算。

    python3 PPBS_B2_D_RUNNER_v1.0.py D1
    python3 PPBS_B2_D_RUNNER_v1.0.py D2
    python3 PPBS_B2_D_RUNNER_v1.0.py D3
"""
import hashlib
...
def main():
    ...
    if which not in ("D1", "D2", "D3"):
        raise SystemExit("用法：PPBS_B2_D_RUNNER_v1.0.py D1|D2|D3")
    frozen = json.load(io.open(INPUTS, encoding="utf-8"))
    env = DC.load_env(ENV)

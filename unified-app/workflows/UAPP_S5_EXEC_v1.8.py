#!/usr/bin/env python3
"""Execute Gate v1.7 in the Founder-authorized CAP-05-first phase order."""
...
GATE = os.path.join(UAPP_ROOT, "stages", "UAPP_S5_GATE_v1.7.json")
MANIFEST = os.path.join(UAPP_ROOT, "stages", "UAPP_S5_CANDIDATE_RUN_MANIFEST_v1.7.yaml")
EVIDENCE = os.path.join(UAPP_ROOT, "evidence", "stages", "uapp_s5_inline_artifact_v1_0", "formal")
RESULT = os.path.join(UAPP_ROOT, "stages", "UAPP_S5_RESULT_v1.5.json")
MATRIX = os.path.join(UAPP_ROOT, "stages", "UAPP_S5_AC_MATRIX_v1.5.json")
PHASE_HEAD = ("UAPP-CAP-05", "UAPP-CAP-06")
...
RUNNER = load_module("uapp_s5_runner_for_v18", os.path.join(HERE, "UAPP_S5_RUN_v1.0.py"))
CHECKER = load_module("uapp_s5_checker_v12_for_v18", os.path.join(HERE, "UAPP_S5_VERIFY_v1.2.py"))
...
def frozen() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if gate.get("document", {}).get("id") != "UAPP_S5_GATE_v1.7":
...
def predecessor_context(
    ...
    return "", f"uapp-s5-inline-v1-{str(turn['conversation_group']).lower()}-20260830"


RUNNER.predecessor_context = predecessor_context
_base_preflight = RUNNER.preflight


def phase_order(scenarios: dict[str, Any]) -> list[str]:
    return list(PHASE_HEAD) + [key for key in scenarios["formal_order"] if key not in PHASE_HEAD]


def preflight(key: str) -> dict[str, Any]:
    result = _base_preflight(key)
    scenarios, _ = frozen()
    order = phase_order(scenarios)
    if key not in order:
        raise RuntimeError(f"Unknown phase-order key: {key}")
    required = order[:order.index(key)]
    prior = {
        item: os.path.exists(check_path(item))
        and RUNNER.load_json(check_path(item)).get("verdict") == "PASS"
        for item in required
    }
    result["authorized_phase_order"] = order
    result["phase_order_prior_checks"] = prior
    result["all_prior_turns_pass"] = all(prior.values())
    result["checks"]["prior_turns_pass"] = result["all_prior_turns_pass"]

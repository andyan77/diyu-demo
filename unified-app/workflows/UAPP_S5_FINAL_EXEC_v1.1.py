#!/usr/bin/env python3
"""Successor executor fixing only frozen checker module/write plumbing."""
...
GATE = os.path.join(UAPP_ROOT, "stages", "UAPP_S5_GATE_v2.1.json")
...
PARENT_GATE_SHA256 = "306b4f29ad403e62582991a66095afd1f73e79a6e5ef69b9da7e179fd0aae515"
...
BASE = load_module(
    "uapp_s5_final_exec_v10_parent", os.path.join(HERE, "UAPP_S5_FINAL_EXEC_v1.0.py")
)
RUNNER = BASE.RUNNER
...
def raw_path(key: str) -> str:
    return os.path.join(EVIDENCE, "raw", f"{BASE.safe_key(key)}.json")


def check_path(key: str) -> str:
    return os.path.join(EVIDENCE, "checks_v1_1", f"{BASE.safe_key(key)}.json")
...
def frozen() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if gate.get("document", {}).get("id") != "UAPP_S5_GATE_v2.1":
        raise RuntimeError("Unexpected successor Gate")
...
def preflight(key: str) -> dict[str, Any]:
    ...
    existing_parent_raw = key == "UAPP-WITHDRAW-01:W0" and os.path.exists(raw_path(key))
    checks = {
        ...
        "raw_absent_or_inherited_w0": not os.path.exists(raw_path(key))
        or existing_parent_raw,
        "successor_check_absent": not os.path.exists(check_path(key)),
...
def verify(key: str) -> dict[str, Any]:
    ...
    expected_gate = (
        PARENT_GATE_SHA256 if key == "UAPP-WITHDRAW-01:W0" else RUNNER.sha256_file(GATE)
    )
...
def main() -> int:
    ...
    if args.preflight:
        ...
        if args.turn == "UAPP-WITHDRAW-01:W0" and os.path.exists(raw_path(args.turn)):
            raise RuntimeError("Inherited W0 RAW must not be rerun")

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""PP 边界后继｜Phase B 把后继 Skill 载入 PP app 并发布新版本。**零模型调用。**
...
provider 仍指向 2026-08-29 03:34:58.999575，Seam 与 M5 在 D1/D2 期间仍走旧 PP。
    ...
    python3 PPBS_APPLY_AND_PUBLISH_v1.0.py --dry-run
    python3 PPBS_APPLY_AND_PUBLISH_v1.0.py --apply
"""
import argparse
import hashlib
...
def main():
    ...
    rep = {"document": {"id": "PPBS_APPLY_AND_PUBLISH_v1.0",
    ...
    if a.apply:
        ...
        st, pub = console.call("POST", "/console/api/apps/%s/workflows/publish" % PP_APP, body={
            "marked_name": "pp-b1-boundary",
            "marked_comment": "事实来源必须蕴含该主张；cta_contract 原文优先于 cta_level 与默认互动方法"},
            timeout=900)
    ...
    io.open(os.path.join(EVDIR, "PPBS_APPLY_AND_PUBLISH.json"), "w",

...
| `DIYU-V1-THREE-SKU-PRODUCTIZATION-001` | **已终结 `DONE`**（见 §一.21；P0/P1/P1.5 三 SKU 产品化改造完成，E2-P 补丁已续接并关闭 P-2）。E5 独立静态审查曾推翻 E3 对 P0 的 READY 判定（32 条 P0 级确定性缺陷）；E6 首轮判 `READY_FOR_EMPIRICAL_TESTING` 后 Founder 复核改判 `NOT_READY`（B-1~B-3 已处置）；E7 再改判 `BLOCKED_BY_STATIC_VALIDITY_DEFECT`（五条必修 M-1~M-5 + 三条顺手修 C-1~C-3），全部处置并整体重跑，三 SKU 回到 `READY_FOR_EMPIRICAL_TESTING`——见 §一.21 E6/E7 段。**E4 v2.0（`DIYU-V1-P0-EMPIRICAL-R1-001`，同分支续接）阶段 A/B 已完成，Founder 2026-09-03 已就阶段 B 的 `thinking` 字段不一致裁决，阶段 C 进行中**：阶段 A 七项已完成并 commit+push。阶段 B 首次真实调用途中现场发现并各自修复两个真实缺陷（Dify 发布前 End 节点输出变量名撞名 `dec7f61`；`envelope_check` docstring 无效转义序列触发 Dify 沙箱判失败 `9eae1bc`，均已 push），第三次调用成功（34526 tokens，Case1/low/k1）；截获真实 payload 与 `_actual_completion_params()` 模拟比对：键集合一致，但 `thinking` 字段真实值 `{"type":"enabled"}` 与模拟值布尔 `True` 不一致，按 Prompt 已停在此等裁决。**Founder 裁决**：该差异不构成需要重估 SG5 结论的不一致（转换发生在剥离判定之前，剥离判据本身两边一致），**但**逐分支核对后发现 `_actual_completion_params()` 另有三条未镜像分支（thinking 缺省/false/dict 形态，两条会咬人），裁决放行阶段 C 前先修复：①`_actual_completion_params()` 改为忠实镜像真实函数全部分支——已完成；②SG6 补三个新检测器 `plugin_normalization_thinking_absent/disabled/dict_enabled` 三控覆盖——已完成，24/24 SG6 三控 PASS；③回填 `SG7.real_provider_payload_verification` 的 `case_id`——已完成；另新增 `STATIC_GATE_REPORT.json` 顶层 `sg14_execution_parity_gap` 登记（`dec7f61`/`9eae1bc` 两个已知实例，均已补齐检测器，如实登记裸 compile/exec 与真实沙箱的结构性差距为已知盲区）。整体重跑无回归（P0 仍 `READY_FOR_EMPIRICAL_TESTING`，17/17 R0 fixture PASS）。**阶段 C（3 案例×2 档位×k=3=18 次真实付费调用）进行中**——见 §一.21 E4 阶段 A/B/裁决段。产物 `sku-productization/static_gate.py` + `fixtures/`（17 个文件）+ `STATIC_GATE_REPORT.json` + `eval/` + `p0-empirical-r1/` | [L1 §T-016.6](L1_TASK_MANIFESTS.md) · [p0-static-audit/STATIC_AUDIT_REPORT.md](../p0-static-audit/STATIC_AUDIT_REPORT.md) · [sku-productization/STATIC_GATE_REPORT.json](../sku-productization/STATIC_GATE_REPORT.json) | `task/three-sku-extraction-v1` 分支起分支；任务分支 `task/three-sku-productization-v1` |
...
**E3 Static Gate（`DIYU-V1-STATIC-GATE-001`，2026-09-02，同分支续接，不新开任务）**：A-1/A-2 已执行（`component_return` 终点改为面向客户三段式、`return_id`/`envelope_hash` 链随之清零，P-2 因此关闭；顾问性②③自检项已补入三份 SKILL_v2.0.md 并 `resync_prompts.py` 同步）。Gate 脚本 `sku-productization/static_gate.py`，唯一证据 `sku-productization/STATIC_GATE_REPORT.json`（可由源码重算，不另立清单）。首轮终态：**P0 `NOT_READY`、P1 `NOT_READY`、P1_5 `NOT_READY`**——三者共同且唯一的阻断项是 SG5 检出的 `thinking:true` 会被 deepseek 插件静默剥离 `temperature`/`top_p` 声明，声明与实际生效参数不一致。过程中另发现并已修复 P1/P1.5 的 Layer-A 同型缺口（`ref_projection` 此前只产出规则文字、未真正字节嵌入 platforms/industry-conditions/examples 三份参考文件，与 B-03 修复前的 P0 同型）——已移植 P0 的构建期字节快照模式并现场核验字节一致，图结构 `16/16` 不变。

**Founder 续裁（同一 `RULESIDE-2026-09-02-014`）**：非二选一——`temperature`/`top_p` 在 `thinking:true` 下本就是无效声明，删声明、保留 `thinking`，两者不冲突。六处位置：P0 两处（含 `temperature`+`top_p`，`skill_llm`/`component_return` 节点各一）、P1 两处（`top_p`）、P1_5 两处（`top_p`），均已删除被剥离参数，保留 `max_tokens`/`reasoning_effort`/`thinking`，运行时行为不变（原本就未生效）。另确认：同输入多次运行输出变异属实测运行参数（k≥3 次评分门），已被该裁决登记为已决事项，不进入 Gate、不登记 `DYNAMIC_ONLY`；`reasoning_effort: low`（六处均最低档）按索引 §五放行，记入报告备注 `reasoning_effort_note`，不阻断。
...
**E6 P0 根因修复与统一回归（`DIYU-V1-P0-ROOT-REMEDIATION-001`，2026-09-02，同分支续接）**：按冻结顺序 R0→R6→R2→R1→R3→R4→R5→R7→一次整体重跑 执行，唯一产物 `sku-productization/static_gate.py`（扩充，未新建）＋ `sku-productization/fixtures/`（Founder 冻结的 14 条失效机制级 fixture，逐条可执行复算）＋ `sku-productization/STATIC_GATE_REPORT.json`。**终态：P0/P1/P1_5 三者均 `READY_FOR_EMPIRICAL_TESTING`，`blockers: []`**（P1/P1_5 未改动 DSL，只因 R4 新增的 `SG5.skill_source_hash_declarations` 检测器起初对三 SKU 通用生效而短暂误报，已改为按 system prompt 自身声明的路径现读、不再硬编码 P0 专属路径，复测两者仍绿）。R6 用 14 条 fixture 现场攻击 E5 报告点名的检测缺口：`SG2`（gate_sufficiency 真假分支反接）、`SG5.assembled_prompt_matches_skill_md`（提示词尾部篡改检测此前的自我复制式判据恒真）经 git 历史复原旧代码现场重跑证实确实被打穿，已修（新增 `SG2.guard_branch_semantics`、拆分为独立冻结 `EXPECTED_PROMPT_TAIL_SHA256` 核验）；`SG3.dangling_template_reference`、`SG5.embedded_reference_bytes`、`SG5.skill_source_hash_declarations` 此前不存在（不是"打穿"，是从无到有）。全部 6 个新增/改造检测器均补齐 SG6 三控自测（正/负/off-list），R0 的 14 条 fixture 中 4 条（FX05/12/13/14）即以"引用对应 SG6 自测通过"的方式复用同一份检测实现，不新造第二套判定逻辑。R2 准入判据改为语义锚点识别（保留结构化 `key: value` 向后兼容），fixture 1/2 验证自然语言正常输入通过、占位符输入被拒；R1 把 `local_block`／`fact_check_mismatch`／新增的 `release_decision_blocks_delivery`（此前不存在，本轮新增解析）接入 `delivery_finalize` 交付判定，fixture 9/10/11 验证均能拦截；R3 删除 `recovery_llm` 节点与其 2 条边，`projection_gate` 两分支现均直连 `delivery_finalize`，图从 16 节点/16 边减至 15/15；R4 修复 SG5 只核对磁盘源文件、从未核对真正嵌入字节这一处，另核对了 E5 点名的"第二处同类问题"（`source_skill_sha256`/`m4_v1_3_successor_sha256` 自报哈希此前无人核对）；R5 给 `app.name`/`app.description`/`skill_llm` 节点标题写入 v2.0 标识，`returns_adapter` 新增 `dsl_version` 字段随每次运行输出，Gate 报告新增按 SKU 记录的 `dsl_sha256`；R7 未扩充市场断言词表，`LEAK_PATTERNS`/`LEAK` 改为大小写不敏感匹配，报告新增 `static_detector_capability_notice` 收窄检测器能力声明措辞，`semantic_market_claim_coverage` 因 `eval/` 无真实 case 可绑 `empirical_case_ref`、按 §13.3 只披露不注册（避免注册后被判 `BLOCKING`）。**两处已知未做、本轮未关闭**：① Q-COMM-04 §1.3 的"12 项标准交付物齐全性"仍无任何代码检查（E5 原文即指出"没有一行代码去检查"）；② R2 prose 里"CTA 分级要么接判断分支、要么至少有代码核对交付内容是否符合声明授权级别"这一条，本轮只做了来源隔离（fixture 3）与非法枚举显式标记（fixture 4），未做"交付内容与声明的 CTA 级别是否一致"这一层检测——Founder 冻结的 14 条清单未包含该项，未新造检测器，如实登记为未验证，不记为已关闭。逐条结论出处见本次会话回执，不重复贴代码。

**Founder 收口纠偏（同日，`DIYU-V1-P0-ROOT-REMEDIATION-001` 续接，未开新任务）**：首轮提交（`4fa4561`）已按 Founder 明确指令 commit+push（工作区曾 1311 行改动未受版本控制保护）。Founder 复核后判首轮终态改判 `NOT_READY`，给出三条必修 blocker，均已处置，整体重跑：

- **B-1a**：Q-COMM-04 §1.3 十二项标准交付物齐全性——新增 `returns_adapter._standard_deliverables_missing()`，字段名取自 SKILL 上文档明的内部 Artifact 结构（不新造判据），只查"是否被提到过一次"（内容或 `NOT_APPLICABLE` 均算），不判内容质量；命中并入 `local_block`。新增 fixture `FX15`（原 14 条冻结清单之外，Founder 本次直接授权追加，非执行侧自行扩容）。
- **B-1b**：CTA 声明级别与交付内容一致性——`envelope_check.cta_level` 接入 `returns_adapter` 新增入参，仅 `cta_level=NO_CTA` 时启用禁止动作词表（词表取自 SKILL 上文"无 CTA 时评论区能做什么"一节明文列出的原句，非新造）；命中并入 `local_block`，负控（授权为 `HIGH_RISK` 时同词表不得误判）已验证。新增 fixture `FX16`。R0 由 14 条增至 16 条，超出原冻结上限 1 条，理由：Founder 本次直接指令新增，非执行侧自行扩容。
- **B-2**：`SG7.semantic_market_claim_coverage` 由"仅披露"改为正式登记 `DYNAMIC_ONLY`，`empirical_case_ref` 绑定判分位 `Q-COMM-04 §6.1/G1/Unsupported_Current_Market_Claim = 0`；`case_id` 现为占位 `PENDING_EVAL_MANIFEST`（`eval/` 仍无真实测试集，未伪造具体案例），待建成后须回填，不得长期占位。
- **B-3**：`SG1.G0.single_highest_value_gap_no_fabrication` 此前靠 `component_return` 源码字符串匹配判定（同属 fixture 13 那类"挖空逻辑留注释即可骗过"的漏洞），改为真实 exec 该节点代码、两组不同 `missing[]` 顺序调用、断言问题随首位缺口真的变化、真的输出三段式、真的不构造 `return_id`——检查行为，不检查源码文本；新增 `SG6.component_return_behavior` 三控自测。`component_return.main()` 签名三 SKU 不完全一致（P1_5 多一个 `entry_resolved`），检查函数改用 `inspect.signature` 动态取参，不假设签名。
- **顺手确认**：`recovery_llm` 删除后 `projection_gate` 两分支确认已殊途同归（均直连 `delivery_finalize`），是恒真路由门；`needs_projection` 的判断本身仍由 `delivery_finalize` 内部读取生效，不受影响——已清理 `projection_gate` 节点本身，`returns_adapter` 直连 `delivery_finalize`，图 15 节点/15 边减至 14/14。

整体重跑（非单项）：P0/P1/P1_5 三者仍 `READY_FOR_EMPIRICAL_TESTING`，`blockers: []`；P1/P1_5 DSL 本轮同样未改动一个字节，改动集中在 `static_gate.py`（新增行为检查、DYNAMIC_ONLY 注册）与 P0 的 `returns_adapter`/图结构。第二次提交+推送，远程指针核验一致。

**E7 P0 残余确定性缺陷修复（`DIYU-V1-P0-RESIDUAL-REMEDIATION-001`，2026-09-02，同分支续接）**：Founder 在 `4e22738` 真实代码上现场复核 + 规则侧独立执行复现，给出五条必修（M-1~M-5）与三条顺手修（C-1~C-3），终态先由 Founder 改判 `BLOCKED_BY_STATIC_VALIDITY_DEFECT`（E4 继续暂停），处置后整体重跑。**本轮第一位的是"变体兄弟规则"**（索引 §13.2 `off-list constructed bad case` 从检测器身上搬到 fixture 身上）：每条修复必须同时验证被描述的攻击实例与至少一个变体（位置换端/大小写全半角/空值占位符/顺序颠倒），不得只验证被描述的那一个。

- **M-1**（`release_decision` 必须出现在用户正文才算数）：`fact_verification` 新增 `A_OPEN/A_CLOSE`，`RELEASE_RE` 改为分别在 ARTIFACT 块与 USER_DELIVERY 块内独立搜索——USER_DELIVERY 缺失该结论（含标记重复按缺失处理）、或为 `DO_NOT_PUBLISH`、或与 ARTIFACT 自报值矛盾，均阻断；新增字段 `release_decision_user_reported`/`release_decision_mismatch`。变体（否定语境包裹/三枚举值各测一次/ARTIFACT 与正文相反）全部在 `FX11`（8 步）内验证；旧提交 `4e22738` 现场复现确认此前"结论只在 ARTIFACT、正文缺失"一样判定不阻断。
- **M-2**（十二项交付物须非空或显式 `NOT_APPLICABLE`）：`_standard_deliverables_missing` 从纯字段名 `\b%s\b` 存在性查找改为要求同一行给出非空、非占位符（见上/同上/待定等）实质值或 `NOT_APPLICABLE`。变体（有名无值/值为空白符/值为占位符/仅一项为空）在 `FX15`（6 步）内验证；旧提交现场复现确认"十二字段名各占一行、全部无值"此前判定齐全。
- **M-3**（marker 成对唯一性：START 与 END 都要查）：三个解析器（`fact_verification`/`market_claim_scan`/`returns_adapter`）的 `_between` 同一处改——此前只检测 START 重复，第一个 END 之后是否还有下一个 END 从未检测。变体（重复 END/交错/标记字符串出现在正文引用里，及 `market_claim_scan` 自身直接验证）在 `FX08`（5 步）内验证；旧提交现场复现确认"合法正文+第一个 END+违规文字+第二个 END"此前不阻断。
- **M-4**（`FACT_LEDGER=NONE` 但正文含事实性陈述）：`ledger_status=="NONE"` 时对 USER_DELIVERY 做封闭集合高信号扫描（数字+计量单位、中文成数/分数表达），命中即与 NONE 声明矛盾判阻断；开放语义完备性转出 G1，不扩关键词表猜语义。新增字段 `ledger_none_contradicted`/`ledger_none_contradiction_hits`。变体（全角数字/中文数字/数字单位跨空白换行/`PARSE_FAILED` 时同样成立）在新增 `FX17`（6 步）内验证。
- **M-5**（准入跨行渗漏）：`envelope_check._find_scalar` 第二条正则冒号两侧 `\s*`（含换行）改为 `[ \t]*`（不含换行）——此前空的必填项紧跟下一行任意内容，会被 `\s*` 跨行吞并当成本字段的值，`_present()` 专门处理"空值后接续行"的、带 KEYLIKE 防护的续行逻辑因此从未被走到。变体（空值后跟缩进行/空值在文本最后一行/值只有空白符/值是另一字段名裸词次行）在 `FX02`（5 步）内验证；旧提交现场复现确认跨行吞并确实发生。（`cta_contract` 字段自身的语义锚点列表含 "cta" 这一自我碰撞的既存现象，本轮未改，已避开用作变体目标字段，未修复不代表未发现。）
- **C-1**（`industry-conditions.md` 补字节绑定）：按 `subject_domain` 五个分支独立提取模板嵌入内容与磁盘源文件对应段落（`## 标题` 到下一个 `---`），逐段归一化 sha256 比对，五段现场核验一致；新增 `SG5.industry_conditions_reference_bytes` + `SG6` 三控。**实现过程中发现并已自行修正一处路径硬编码**：初版硬编码 P0 的 `packaging-content-for-release` 路径，对三 SKU 通用运行会对 P1/P1_5 校验错误的源文件（三份目前字节相同侥幸未触发误报，但独立漂移时会漏判）——改为按 `sku["skill_source_dir"]` 现读路径，与 R4 当初 `skill_source_hash_declarations` 同一教训。
- **C-2**（INV-1 增加工作区/暂存区未提交改动检查）：`check_inv1` 新增 `git diff`/`git diff --cached` 对 `INV1_FROZEN_TREES ∪ INV1_FROZEN_FILES` 的核验；已用真实改动现场验证（临时改动一个冻结文件，工作区与暂存区各一次，均被捕获，revert 后确认干净）。
- **C-3**（`run_sg1` 的 `fact_verification`/`market_claim_scan` 节点存在即 PASS 改为行为断言）：与 B-3 同一手法，真实 exec 节点代码、构造输入调用 `main()`，只用三 SKU 共有字段（`fact_gate_blocked`/`market_claim_blocked`），因为 P1/P1_5 这两个节点本轮未改动、检查必须对三者通用——已对三份 DSL 分别现场验证正/负/off-list 三控。新增 `SG6.fact_verification_behavior`/`SG6.market_claim_scan_behavior`。
- **转出四项登记**：①真实 Provider Payload——唯一按原文明确要求"登记 `DYNAMIC_ONLY`"的一项，新增 `SG7.real_provider_payload_verification`，绑 E4 首次真实调用；②"`missing[0]` 是否真的最高价值"——属产品判断，未编造 Q-COMM-04 判分位，只在报告 `residual_remediation_transferred_out_note` 字段指到 `SG1.G0` 现有登记位置；③事实性陈述开放语义完备性——已在 M-4 分层说明与 `fact_check_scope_note` 字段登记；④`subject_domain` 同义表达——不做开放世界同义匹配，本轮未加 canonical alias（不知道 E4 实际会用哪三个案例），登记为已知限制。
- **E6 遗留的两处"已知未做"本轮关闭**：①十二项标准交付物齐全性→M-2 已实现；②CTA 声明与交付内容一致性→已在 E6 收口纠偏 B-1b 关闭（E7 未重复处理，E6 段历史文本不改）。

**产物**：`sku-productization/fixtures/` 17 个文件（13 `node_exec` + 4 `gate_selftest`，未超 16–18 上限；仅新增 `FX17`，其余靠变体用例内补），共 44 个 chain 步骤（用例实例）、54 条断言；`static_gate.py` 扩充（未新建检查体系）；P1/P1_5 DSL 本轮未改动一个字节（`git diff --stat` 确认）。整体重跑：**P0/P1/P1_5 三者 `READY_FOR_EMPIRICAL_TESTING`，`blockers: []`**。已 commit + push，远程指针核验一致。

**E4 v2.0 阶段 A · 零调用收口（`DIYU-V1-P0-EMPIRICAL-R1-001`，2026-09-03，同分支续接）**：七项全部做完。A-1 收窄 `end_ok`（删除 `artifact`/`raw_preserved` 两项从未被 market_claim_scan/LEAK_PATTERNS 扫描过的输出）+ `end_component_return.returns_json.precise_gap` 改用可读标签替换内部字段名（此前会把 `content_body_or_beats` 等内部标识原样拼进对外 JSON，`leaked` 自检当时只查 `user_delivery`，现覆盖 `returns_json`）。A-2 精确枚举 BLOCKING-capable 检测器：19 个已三控（本轮新补 6 个：`advisory_marker_present`／`applicable_not_applicable_present`／`unknown_fact_misuse_scan`／`guard_variable_source_hardcoded`／`reference_provenance_check`／`evidence_and_authority_check`），1 个非 BLOCKING-capable（`SG5.no_dependency_on_frozen_old_system` 硬编码恒 PASS，无需三控），4 个本轮未补齐并如实登记原因（`SG7.real_provider_payload_verification`/`SG7.semantic_market_claim_coverage` 自身不直接产出 BLOCKING、唯一路径是 SG4 的 auto-escalation、已被 SG4 自身三控的 off-list 覆盖；`INV-1` 操作真实 git 状态而非 `yml_data`，本轮用真实改动手动验证代替自动化三控；`INV-2` 的密封守卫在模块导入时一次性永久安装，同进程内无法构造"守卫不存在"的负控，结构性豁免）。A-3 正式核销 E5 §三全部条目（S0/S1–S14 + 未知盲区扫描新发现）+ §二第8组，`STATIC_GATE_REPORT.json` 新增 `e5_residual_written_off` 字段（范围/三条理由原文照抄/裁定人 Founder/日期 2026-09-02），原文标题原样留在 `p0-static-audit/STATIC_AUDIT_REPORT.md` 未删除。A-4 给 `subject_domain`（此前无任何语义锚点、只认结构化声明，比 `platform` 更弱）补三个案例实际涉及行业的自然语言锚点（餐饮/门店、知识付费/课程、户外/露营），其余行业维持现状。A-5/A-6 冻结三案例入 `eval/`（`EVAL-P0-R1-001~003`）：案例1（餐饮/抖音，含一句真实商业语言写的、不落在 `MARKET_CLAIM_PATTERNS` 固定模式里的市场地位断言，专门实测 `SG7.semantic_market_claim_coverage` 披露的同义改写风险）、案例2（知识付费/小红书，刻意输入不足，唯一一个）、案例3（户外/视频号，`cta_contract` 全篇零结构化痕迹纯靠自然语言锚点通过）；准入预验：案例1 需为 3/7 必填项补话（`content_body_or_beats`/`facts_registered`/`asset_publish_permission`，首稿用未来时"想拍"而非过去时"已经拍"就会被判缺失）才能通过，案例3 需为 1/7 项补话，案例2 按设计保持不足——**这是 E4 唯一给出的"真实用户自然语言进不进得来"的一手证据，直接印证已登记的 KL-2（封闭词表匹配开放语言）**。A-7 建 `eval/` 并以清单核验方式回填：`check_inv2()` 此前把"sealed root 存在任何内容"一律判 bad（当时确实什么都不该在那儿），现在 `eval/` 合法持有本任务冻结内容，改为 `EVAL_MANIFEST`（路径+sha256，仿 INV-1 同一模式）核验——清单外文件或哈希不符仍判 bad，已用真实改动（塞入一个清单外文件、篡改一份已入清单文件的字节各一次）现场验证捕获、复原后确认干净；`sha256` 计算本身需要绕过 `_guarded_open`（读密封路径的字节去核验哈希，和"不得读密封内容"字面冲突），改用模块加载时捕获的 `_real_open`，仅此一处、仅用于按清单核验已知文件，不读任何清单外内容。`SG7.semantic_market_claim_coverage.case_id` 从占位符 `PENDING_EVAL_MANIFEST` 回填为 `EVAL-P0-R1-001`。整体重跑：P0/P1/P1_5 三者仍 `READY_FOR_EMPIRICAL_TESTING`，`blockers: []`；P1/P1_5 DSL 本轮仍未改动一个字节。**已 commit + push**，阶段 B（首次真实付费调用，截获 payload 与模拟比对）尚未开始。

**E4 v2.0 阶段 B · 首次真实付费调用（2026-09-03，同分支续接）**：三次尝试。第一次在 Console 发布时被 Dify 自身发布前校验拦下——`end_ok`/`end_component_return` 两个 End 节点声明了同名输出变量（`user_delivery`/`user_delivery_leaks`），这条平台侧约束 `static_gate.py` 此前从未建模；改名 `end_component_return` 的两项输出为 `component_user_delivery`/`component_user_delivery_leaks`（value_selector 不变，纯改名，End 节点是终点无内部读者），新增通用检测器 `SG3.end_node_output_name_uniqueness` + 三控自测，commit `dec7f61`。第二次真实调用（Case1/low/k1）到达 `envelope_check` 节点即失败，零成本：`_find_scalar` 的 docstring（E7 M-1 时新增的说明文字）是非 raw 三引号字符串，内含未转义 `\s`，编译期 `SyntaxWarning: invalid escape sequence '\s'`，Dify 真实沙箱执行器把这类警告当节点失败处理，本地裸 `compile()`/`exec()` 从不检查警告故此前不可见；用 `git show HEAD` 回放确认修复前代码确实且仅触发这一条警告，修复为 docstring 加 `r` 前缀，P1/P1_5 现场核验零同类警告（P0 专属新增文字，非跨 SKU 缺陷）；新增通用检测器 `SG3.code_node_compiles_without_warnings`（对全部 code 节点逐个 compile 并捕获警告）+ 三控自测（正控当前 DSL／负控注入 `\s`／离表负控在另一节点注入不同的 `\d`），commit `9eae1bc`。第三次调用成功：`workflow_finished status=succeeded`，34526 tokens（prompt 18460 + completion 16066），耗时 174.29s，`workflow_run_id=1beff598-8602-46ab-b35c-e767e8962866`。**截获比对结果：不完全一致**——Dify 的 SSE 响应与数据库 `workflow_node_executions.process_data` 均不落 `model_parameters`/completion_params 本身（只有 `model_mode`/`prompts`/`usage`/`finish_reason`/`model_provider`/`model_name`，`process_data_truncated=false`，确认不是截断而是本就不记），plugin_daemon 的 `docker logs` 与容器内 `/app/storage/logs/server.log` 在当前 `LOG_LEVEL=INFO` 下也只有一行结构化摘要、不含请求体，因此无法从 Dify 自身任何记录里直接拿到出站字节；改为从活的 `plugin_daemon` 容器 `docker cp` 出该 tenant 实际绑定版本（`docker exec` 查询 `dify_plugin.plugin_installations` 确认 `langgenius/deepseek:0.0.20@850efe73...`，与 Prompt 引用基线一致，非另一缓存的 0.0.21）的两份真实源码（`models/llm/llm.py` 与 SDK `openai_compatible/llm.py`），从第一份里正则提取 `_normalize_model_parameters` 方法体原文（非重新实现）用 P0 真实声明的 `completion_params`（`max_tokens: 384000, reasoning_effort: low, thinking: true`）实际执行，并读第二份确认 `data = {model, stream, **model_parameters}` 后原样 `json.dumps` 进请求体、`thinking` 之后不再被改动——真实执行结果 `thinking` 被转成 `{"type": "enabled"}`，`static_gate.py._actual_completion_params()` 的模拟结果仍是原样布尔 `True`，键集合完全一致（两边都认定当前声明不含 temperature/top_p/presence_penalty/frequency_penalty、无键被静默丢弃，`SG5`/`SG7._plugin_normalization_mismatch()` 唯一在用的判据未受影响），但 `thinking` 这一个字段的取值表示确认不一致。按 Prompt 原文"不一致→立即停止，不要跑对照…报告差异，等裁决"，**阶段 C 未发起**，证据落盘 `p0-empirical-r1/PHASE_B_PAYLOAD_VERIFICATION.json` + `p0-empirical-r1/raw/`（原始 SSE、两份真实插件源码及其 sha256）。`sku-productization/static_gate.py` 里 `SG7.real_provider_payload_verification` 的 `EMPIRICAL_CASE_REF_PROVIDER_PAYLOAD`（`case_id: PENDING_E4_FIRST_CALL`）本轮**未改动**——是否把这次发现的字段级不一致判定为需要重估、以及重估结论如何写回该登记项，留给 Founder 裁决，不由执行侧自行改判。commit `dec7f61`/`9eae1bc` 均已 push，`p0-empirical-r1/` 尚未提交。

**Founder 裁决（2026-09-03）与阶段 B 收口**：裁决全文见 `p0-empirical-r1/PHASE_B_PAYLOAD_VERIFICATION.json` 新增字段 `founder_ruling_2026_09_03`。结论：`thinking` 表示差异（真实 `{"type":"enabled"}` vs 模拟布尔 `True`）不构成需要重估既有 SG5 结论的不一致——该转换发生在剥离判定之前，转换后立即以 `type=="enabled"` 触发剥离，SG5/SG7 唯一在用的判据（键集合差异）两边结论相同；当前三份 DSL 均声明布尔 `thinking:true`，走的正是这条已验证一致的路径。但 Founder 逐分支核对真实 `_normalize_model_parameters()` 后指出，旧 `_actual_completion_params()` 只镜像了 `thinking:true` 一支，另外三支（缺省/`false`/dict 形态）均未镜像且其中两支会咬人：缺省应剥四项、`false` 应只剥 `reasoning_effort`，旧实现两者都不剥；根因是旧代码用 `declared.get("thinking") is True` 判定，缺省或非 `True` 布尔值时一律落到"什么都不剥"，而真实代码首行是 `model_parameters.get("thinking", True)`，缺省即视为 `True`。裁决要求阶段 C 前先修复三项，本轮已全部完成：①`_actual_completion_params()` 改为完整镜像真实函数全部分支（缺省→True、bool→dict 转换、disabled 分支只剥 `reasoning_effort`、dict 形态直接按 `type` 判定）；②SG6 新增三个检测器 `plugin_normalization_thinking_absent`/`_thinking_disabled`/`_thinking_dict_enabled`，各自 pos/neg/offlist 三控直接断言 `_actual_completion_params()` 的精确键集合（而非只用粗粒度的 `_plugin_normalization_mismatch()` 布尔值，因为后者无法区分"剥对了键"与"剥错了键"）——用 `git show HEAD` 回放旧实现确认三者在修复前确实都会误判（缺省/`false`/dict 场景下一律不剥，与真实行为不符），修复后 24/24 SG6 三控全绿；③`EMPIRICAL_CASE_REF_PROVIDER_PAYLOAD` 的 `case_id` 回填为 `EVAL-P0-R1-001/low/k1 · workflow_run_id=1beff598-8602-46ab-b35c-e767e8962866`，`SG7.real_provider_payload_verification` 就此闭合。另按 Founder 指示，`dec7f61`（End 节点输出变量名全局唯一）与 `9eae1bc`（SyntaxWarning 即节点失败）正式登记为 `STATIC_GATE_REPORT.json` 顶层新增字段 `sg14_execution_parity_gap`（对应索引 §13.2 SG14）——如实记录 `static_gate.py` 用裸 `compile()`/`exec()` 而非真实 Dify 沙箱执行 code 节点这一结构性差距，两个已知实例均已补齐检测器，但如实登记为"不反证不存在第三个未知分歧点"的已知盲区，不声称已穷尽真实沙箱的全部拒绝条件。整体重跑：P0 仍 `READY_FOR_EMPIRICAL_TESTING`，P1/P1_5 仍 `NOT_READY`（唯一原因不变，仍是已披露的 `end_node_output_name_collisions`），17/17 R0 fixture PASS，24/24 SG6 三控 PASS，无回归。阶段 C（3 案例×2 档位×k=3=18 次真实付费调用）自此正式发起。

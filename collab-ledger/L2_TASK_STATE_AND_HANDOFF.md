...
| `DIYU-V1-M2-BUSINESS-PERSISTENCE-VERSION-FEEDBACK-001` | [M2 施工 Execution Prompt v1.1](../decision-chain/docs/M2_ENGINEERING_EXECUTION_PROMPT_v1.1.md) + [Rebase/Errata 001](../M2_ENGINEERING_EXECUTION_PROMPT_v1.1_REBASE_ERRATA_001.md) | 本任务自身 Task Contract（内嵌于原 Prompt §3，独立复算 `task_contract_hash = 4d14eb35c065b650b0380b0c309e0e08ec32e3aa608ece4d62e8d27b97450830`，见 [L1 §T-011](L1_TASK_MANIFESTS.md)） | **状态更新（2026-08-25，Rebase/Errata 001 后，取代上一条状态更新）**：Rebase 复验发现此前"AC-00~16 全部 PASS"的判定不准确——`M2-AC-12`（并发裸 500）本轮已真实修复（此前误披露为"已知限制"）；`M2-AC-13`（数据库权限）与 `M2-AC-16`（Dify 画布证据）经现场实测**下修为 NOT_VERIFIED**（前者：`diyu_app` 实际可 CONNECT 到 `dify`/`dify_plugin`，表级读取仍被拒绝；后者：无可用 Dify 会话，未重新真实运行画布）。Checkpoint = `IN_PROGRESS`（**不是** `AWAITING_FOUNDER_DIFY_ACCEPTANCE`）。**下一动作**：(a) 需要 Founder 或有权限者手动执行 `REVOKE CONNECT ON DATABASE dify, dify_plugin FROM PUBLIC, diyu_app`（修复动作本身被权限分类器拦截）；(b) 需要一次可访问真实 Dify 1.16.1 的会话来重新真实运行候选画布；两者之一或都完成后才能重新评估是否进入 `AWAITING_FOUNDER_DIFY_ACCEPTANCE`。逐条证据见 `business-persistence/M2_ACCEPTANCE_EVIDENCE.md`，Rebase 完整记录见 `business-persistence/M2_REBASE_ERRATA_001_RECORD.md` | 独立 worktree／任务分支 `task/m2-business-persistence-version-feedback-v1`（已建立，见 [L3 §十三](L3_ATTEMPTS_AND_EVIDENCE.md#十三-diyu-v1-m2-business-persistence-version-feedback-001)） | `main @ 3fcfac02365b398593634f0449921f4775f06af4` ＋ 该 Prompt 全文 | `M2-AC-00` 至 `M2-AC-17` 全部 PASS ＋ 提交 Founder Dify 实测包，见该 Prompt §9/§12——`AC-13`/`AC-16`/`AC-17` 待完成 |

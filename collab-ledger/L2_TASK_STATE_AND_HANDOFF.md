...
| `DIYU-V1-THREE-SKU-PRODUCTIZATION-001` | **已终结 `DONE`**（见 §一.21；P0/P1/P1.5 三 SKU 产品化改造完成，E2-P 补丁已续接并关闭 P-2）。E5 独立静态审查曾推翻 E3 对 P0 的 READY 判定（32 条 P0 级确定性缺陷）；E6 首轮判 `READY_FOR_EMPIRICAL_TESTING` 后 Founder 复核改判 `NOT_READY`（B-1~B-3 已处置）；E7 再改判 `BLOCKED_BY_STATIC_VALIDITY_DEFECT`（五条必修 M-1~M-5 + 三条顺手修 C-1~C-3），全部处置并整体重跑，三 SKU 回到 `READY_FOR_EMPIRICAL_TESTING`——见 §一.21 E6/E7 段。**E4 v2.0（`DIYU-V1-P0-EMPIRICAL-R1-001`，同分支续接）现在进行中**：阶段 A 零调用收口七项（A-1~A-7）**已全部完成并 commit+push**，`eval/` 已建成第一份实测集（`EVAL-P0-R1-001~003`），三 SKU 仍 `READY_FOR_EMPIRICAL_TESTING`；**阶段 B（首次真实付费调用）与阶段 C（18 次档位对照）尚未开始**——见 §一.21 E4 阶段 A 段。唯一产物 `sku-productization/static_gate.py` + `fixtures/`（17 个文件）+ `STATIC_GATE_REPORT.json` + `eval/` | [L1 §T-016.6](L1_TASK_MANIFESTS.md) · [p0-static-audit/STATIC_AUDIT_REPORT.md](../p0-static-audit/STATIC_AUDIT_REPORT.md) · [sku-productization/STATIC_GATE_REPORT.json](../sku-productization/STATIC_GATE_REPORT.json) | `task/three-sku-extraction-v1` 分支起分支；任务分支 `task/three-sku-productization-v1` |

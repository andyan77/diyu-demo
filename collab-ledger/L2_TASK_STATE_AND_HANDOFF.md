...
| 未完成 | 14 条快照语义×5 维度的完整落地（当前只有 9 个扁平字段的 P0 切片，`content_task` 投影对这部分缺口已如实标记，未回填内容）；`call_intent` 对 CAP-01～08 的完整必需输入判据；`open_threads` 跨轮延迟表面化语义（需设计判断）；A-1/A-3/A-4(b) 的验收边界判断（需 Founder/独立审查裁定，见 evidence §九）；独立 Reviewer；`M1-AC-00`～`M1-AC-15` 逐项验收 |
| 下一步可立即执行的动作 | 扩展 `m1_context_compiler_v0.1.py` 覆盖设计文档 §二 完整 14 条语义（当前 9 字段 P0 切片，会同步消解 `content_task` 里的多项 `NOT_CAPTURED_IN_P0_SNAPSHOT` 缺口）；把 evidence §九 的 M1-AC-12 部分满足状态提请独立审查/Founder 判断 |

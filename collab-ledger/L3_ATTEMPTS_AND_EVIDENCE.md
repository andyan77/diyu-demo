...
| 已知未完成 | `M1-AC-15` 阻断待解除——需 Founder 亲自执行一次真实版本回退/重发演练，或提供有效控制台凭证交执行侧完成（详见 L2）；**B-3/B-4/B-5 已实现但均未经 live Dify 验证、未经 §8 标准的正式独立审查**（预算已耗尽，只是同会话对抗式自查）；v0.7 尚未导入/发布；此前收口审查对 AC-03/04/07/10/13/14 的 PASS 已因本批源码改动过期，需新一轮 `affected_scope_only` 复验；两处非阻断账本完整性小缺口与 v1.3 哈希自证治理待决项已如实记录，等规划侧/Reviewer 后续核对 |

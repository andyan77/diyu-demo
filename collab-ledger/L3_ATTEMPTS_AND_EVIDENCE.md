...
| 已知未完成 | 14 条语义×5 维度里仍缺数组型/多维度的 4 项（见上）；`call_intent` 完整必需输入判据；`open_threads` 跨轮延迟表面化语义（需设计判断而非当场改动）；A-1/A-3/A-4(b) 的验收边界判断（需 Founder/独立审查裁定）；独立 Reviewer；`M1-AC-00`～`15` 逐项验收 |

...
| **live 复验被阻塞（如实记录，非隐瞒）** | DSL 已用 `build_m1_candidate_dsl_v0.1.py` 重新生成（v0.4，同步影子节点提示词/schema/默认快照 JSON），但导入/发布/真实回归未能执行：`docker ps` 显示 `docker-api-1` 等容器本会话期间发生过重启（created 3 天前、Up 8 小时），推断服务端会话存储被清空；用 `refresh_token` 续期虽返回成功，但续期后的 `access_token` 访问 `/console/api/apps` 仍 401，判定服务端已不承认旧令牌。执行侧未持有 Founder 明文密码，按凭据最小暴露原则不重新索取，未强行绕过。**v0.2 快照扩展目前只有单测证据，未经真实 Dify 端到端验证；候选 App 实际运行的仍是 v0.3**，详见 [`V1_M1_CANDIDATE_RUN_001.md` §十](../decision-chain/evidence/V1_M1_CANDIDATE_RUN_001.md) |
| 已知未完成 | 14 条语义×5 维度里仍缺数组型/多维度的 4 项（见上）；`call_intent` 完整必需输入判据；`open_threads` 跨轮延迟表面化语义（需设计判断而非当场改动）；A-1/A-3/A-4(b) 的验收边界判断（需 Founder/独立审查裁定）；v0.2 快照扩展的 live 端到端验证（被会话阻塞卡住）；独立 Reviewer；`M1-AC-00`～`15` 逐项验收 |

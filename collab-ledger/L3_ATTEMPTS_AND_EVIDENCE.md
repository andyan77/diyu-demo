...
| 已知未完成 | 14 条语义×5 维度完整落地（现只有 9 字段 P0 切片，`content_task` 对该缺口已如实标记，未回填内容）；`call_intent` 完整必需输入判据；`open_threads` 跨轮延迟表面化语义（需设计判断而非当场改动）；A-1/A-3/A-4(b) 的验收边界判断（需 Founder/独立审查裁定）；独立 Reviewer；`M1-AC-00`～`15` 逐项验收 |

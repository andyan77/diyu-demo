...
## 十八、`DIYU-V1-THREE-SKU-EXTRACTION-001`

### ATT-001（一次直达，`DONE`；三 SKU 统一摘取与语义对齐，零调用，摘取用复制不用移动）

- **输入与授权**：治理绑定三份 —— `Q-COMM-04_P0_...验收标准_v1.0.md`（sha256 `55bcfa4001668dd23614459cb502aca30286e62894670a911a68de17528cb397` 现场复算一致）；`Q-COMM-05_P1_...验收标准_v1.0.md`（sha256 `ec6871f011651d282eafab8e55f50229e102cf4234957dfaeee1dbb4be9b69e6` 现场复算一致）；`Q-COMM-06_P1.5_...验收标准_v1.0.md`（sha256 `9a063c6ddb92e7c78494ef38dd0427a6a8fc070b62be8dd683f27e7a93a9a772` 现场复算一致）。被摘对象共 8 份文件（P0/P1/P1.5 各一份 DSL + 一份 SKILL.md + 三份 references）现场复算 sha256 与 bytes 全部与授权值逐字节一致。`笛语商业SKU验收体系_索引与启动规则_v1.0.md` §10、`P0_推进路线图_v1.0.md` 四 SKU Gate 表只读参考，未复算。从 `task/pp-architecture-reverification-v1` 分支起分支，新建任务分支 `task/three-sku-extraction-v1`。
- **方法**：`yaml.safe_load` 结构化解析三份 DSL 全部节点/边，逐节点代码/模板做**程序化字符串相等性比较**（非按节点名字假设相同），确认三者图骨架在 S1-S4 之前完全同构（P1/P1.5 边集合逐字节相同，P0 = 同一骨架 + S1-S4 新增两节点三段边）；`envelope_check`/`returns_adapter`/`component_return`/`delivery_finalize` 四个核心 code 节点剥离 SKU 专属配置常量后逐字节 100% 相同，含 `QUESTION_MAP`/`VACUOUS`/`GOAL_FAMILIES`/`CTA_LEVELS` 等共用常量列表。摘取全程用 `cp` 逐文件复制，每次复制后立即复算 sha256 与源比对。
- **E1-1 三方结构比对**：产出节点骨架对照表、共用件清单（`SHARED_BY_ALL_THREE` 6 项：外壳校验／充分性闸／参考文件投影／交付适配／交付缺失判定与交付收口／保真绑定记录；`SHARED_BY_TWO` 0 项；`SKU_SPECIFIC` 2 项：事实核验／市场断言检测，且均为"P0 已建、另两个未建但设计上可复用"，非天然专属）与差异性质分类（真实专业差异 vs 历史偶然，如 `QUESTION_MAP` 三份逐字节相同却分别维护即为历史偶然的典型例子）。P0 的 v1_4 相对 P1/P1.5 的额外内容（S1-S4 六条修复）已逐处标出，不计入差异分类。详见 [sku-extraction/THREE_WAY_COMPARISON_v1.0.md](../sku-extraction/THREE_WAY_COMPARISON_v1.0.md)。
- **E1-2 产品合同对齐**：三张表逐行判 `MATCH`/`PARTIAL`/`MISSING`/`OUT_OF_CONTRACT`。P0：13/2/0/3；P1：5/4/2/3；P1.5：12/4/1/3。三者均发现 M4 接缝残留基础设施（`return_to_script[]`/`binding_record`/entry-run_mode 协议）不属于各自验收标准的产品合同范围，标为 `OUT_OF_CONTRACT`。P1/P1.5 在事实核验/市场断言检测两项硬门上均为 `PARTIAL`/`MISSING`（与 P0 修复前状态一致，印证是"未走修复流程"而非"天然不需要"）。详见 [sku-extraction/CONTRACT_ALIGNMENT_P0_v1.0.md](../sku-extraction/CONTRACT_ALIGNMENT_P0_v1.0.md) 等三份文件。
- **E1-3 建新树并复制**：`core/`（`guards/` 承接已独立文件化的 `MARKET_CLAIM_PATTERNS_v1.0.json`；`context-compiler/references/` 承接三方确认逐字节相同的三份参考文件代表性副本；**未建 `expert-core/`**——PP-x/CS-x/PD-x 核心判断逐条为各 SKU 专有内容，"两问"框架虽被 P1 SKILL.md 声明为"三份 Skill 共用"但只以各自独立措辞的散文形式存在，未材料化为可复制的独立文件，如实登记"无三方共用专业真源可复制"）、`eval/`（顶层，与 `core/` 平级，仅 `.gitkeep` 占位，暂空）、`products/`（三个 SKU 各自的 DSL + SKILL + references 完整拷贝）、`adapters/`（顶层空目录，`.gitkeep` 占位，**未建 `douyin/`/`xiaohongshu/`**——platforms.md 是单一统一表格，各平台是表内列而非独立模块，未比出可分离的平台适配层）。共复制 19 个文件，全部现场复算 sha256 与源逐字节一致。详见 [sku-extraction/EXTRACTION_MANIFEST_v1.0.json](../sku-extraction/EXTRACTION_MANIFEST_v1.0.json)。
- **E1-4 P2 静态评估（不摘）**：核实 `operating-one-account/SKILL.md`（载体适配版）与 `m3-account-content-operator-semantic-v1.0/skill-source/SKILL.md`（语义真源版）为同一套专业判断的两层，非两份竞争 SKILL。与 Q-COMM-07 差距：Mode A 的诊断职能与证据分级纪律已相当成熟（十类证据身份表比另三个 SKU 更精细），但§1"两态必须分 Gate 验收"这一结构性要求未满足（现有四类行为可自由组合，非互斥状态），且完全无 M4 tool DSL 承接结构性充分性闸。两条路径（重建 M4 形态 / 另起）各自的要做什么与代价已列出，未做取舍——重建 M4 形态可复用三方已验证的共用节点但需处理 Mode B 未来的持久化状态需求与 M4 骨架无状态假设之间的张力；另起可从一开始为持久化留位但放弃已验证代码的复用。详见 [sku-extraction/P2_STATIC_ASSESSMENT_v1.0.md](../sku-extraction/P2_STATIC_ASSESSMENT_v1.0.md)。
- **E1-5 结转清单逐条处置**：CF-01/02/03 不适用于本任务，推迟到阶段三/阶段二相应环节；CF-04（原 FU-01）推迟到 S6，并新发现该项范围因本任务新增 `core/context-compiler/` 副本而从"三处物理副本"扩大为"四处"，处置不变仍推迟；**CF-05（顾问性三条判据）独立静态检查扩展到 P1/P1.5**：三个 SKU 呈现完全一致的缺口分布——①③全部无承载，②的局部承载来自三者共用的同一处"平台数值参数不伪造精度"规则（非各自独立设计），额外发现 Q-COMM-05 §7 M4"条件改变裁决会反转"这一验收标准评分点在 P1 构建物里没有任何承载字段；处置为推迟到"S5 判定之后、阶段三之前"，且指出应按三 SKU 共用方式一次性设计承载机制，不必分别各建一遍。**CF-06（独立仓库拆分时点与禁带项）**：确认 `eval/` 已建在顶层（与 `core/` 平级），满足"E1-3 建树时即放顶层"要求；完整禁带清单推迟到真正触发拆分时逐项过；远程仓库沿用 `andyan77/diyu-demo`，本轮不拆分。详见 [sku-extraction/CF_DISPOSITION_v1.0.md](../sku-extraction/CF_DISPOSITION_v1.0.md)。
- **过程更正**：母 Prompt 中途收到三处补丁——① `eval/` 从 `core/` 下移到顶层、与 `core/` 平级；② 落盘由三棵树改四棵（`core/ eval/ products/ adapters/`）；③ E1-5 由 CF-01~05 扩到 CF-01~06。均已在建树结构与本条、以及交付物中完整体现，未静默处理，未按原始三棵树的旧描述执行。
- **模型调用实际使用**：`0`（本任务全程不调 LLM）。
- **v1_3 冻结基线复算**：开工前与任务结束各复算一次 sha256，均为 `daa8365de26f9b280e2ea72707aa85ce445edd2b8bcdaa54350ecce9797b635e`，逐字节未变；P0/P1/P1.5 全部 8 份被摘原始对象（含 `MARKET_CLAIM_PATTERNS_v1.0.json`）任务结束时同样复算确认逐字节未变，`git status --short` 核对只出现新增条目，未出现对任一原文件的 `M`（modified）标记。
- **终态**：`task_final_status = DONE`。不产生任何产品/商业结论、不评价三 SKU 质量、不改任何 DSL/SKILL/references 内容——发现的差距全部登记不修。


## 十九、`DIYU-V1-THREE-SKU-PRODUCTIZATION-001`

- **ATT-001（唯一尝试，`DONE`）**：在 E1 摘出的三个产品目录（`products/p0-publishing-packaging/`、`products/p1-creative-director/`、`products/p1_5-production-director/`）内做产品化改造，改动落在新增的 `SKILL_v2.0.md`／`*_v2_0.yml` 六个文件，E1 六份基线原样保留不覆盖。
- **开工前治理绑定复算**：`Q-COMM-04`/`Q-COMM-05`/`Q-COMM-06` 三份验收标准 sha256 现场复算与母 Prompt 给出值一致；`sku-extraction/CONTRACT_ALIGNMENT_P0_v1.0.md`/`_P1_v1.0.md`/`_P1_5_v1.0.md` 三份 E1 产出 sha256 现场复算一致；`products/` 下六份 E1 基线文件 sha256 现场复算与 `EXTRACTION_MANIFEST_v1.0.json` 登记值一致。
- **停下来问（AskUserQuestion，一次三问）**：① P1.5 对齐表第 3 项（`subject_domain`/`content_origin_mode[]` 透传给下游）在任务正文①②③举例里未出现，是否算"自行扩大"不敢自行拆——Founder 裁决"本轮一并拆除"，并指出对齐表本身才是权威拆除清单，正文①②③只是举例不是穷举；② P0「局部失效与不反向传播」节里 `stale_set[]`/失效传播表与「发布实例纪律」子节如何切分——Founder 核对原文后确认"表格部分拆，发布实例纪律留"，因后者是 PP 对客户的版本诚信承诺，与通知别的组件无关；③ 删除 `return_to_script[]` 等字段后，正文里依赖它作为解决路径的判断规则（如 PP-3 第 3 步）会变成断链引用，如何处理——Founder 裁决"改写成面向用户的等价表述，判断本身保留"，并给出通用口径"拆的是机制、协议、字段，不是判断"。
- **执行中发现、未被任务正文预先点名的问题**：排查 `returns_adapter`/`delivery_finalize` 代码时发现"Return 闭环"不只是 SKILL.md 里的文字说明——`returns_adapter` 有真实代码解析 `---M4_RETURNS---` 分隔块（`R_OPEN`/`R_CLOSE`/`RET_FIELDS`/`DISPOSITIONS`/`_parse_returns()`），并把解析结果计入 `blocked`（进而影响交付状态）；`delivery_finalize` 会在恢复成功/失败两条路径上主动往 `rets` 里追加 Return 形状的记录。若只删 SKILL.md 文字、不动这部分代码，机制会"名亡实存"——模型不再被要求产出 `---M4_RETURNS---` 块，但代码仍按"缺失即解析失败"的逻辑把每次交付判定为异常。据此把删除范围扩大到三份 DSL 的 `returns_adapter`/`delivery_finalize` 代码本身（三个 SKU 同型重构，仅 `CAPABILITY` 常量不同），并在 `REMOVAL_TRACE.md` 里注明理由，未静默处理。
- **二次残留排查**：完成首轮基于任务正文①②③与对齐表逐条对照的删除后，对三份 `SKILL_v2.0.md` 做了两轮 `grep "Return"`/`grep "return_to_\|entry_resolved\|下游失效"` 全文排查，额外发现并处理：P1「输入」表里遗漏的 `return_from_downstream[]` 整行、CS-6 节内"回改建议通过 `return_from_downstream[]` 回到这里"尾句、CS-4 节"回改走 `return_to_script`"一句、「与统一能力接缝的对接」节里"只做局部 Return"悬空引用；P1.5 PD-4 节"走 `return_to_script[]`"一句、「默认失败模式对照」表里两行专属于已删除 Return 协议的失败案例、自检第 16 条、「与统一能力接缝的对接」节悬空引用。均已在 `REMOVAL_TRACE.md` 逐条登记，未遗漏进最终交付。
- **T2 四条接线要求实测**：编写独立测试脚本，从 `DIYU_M4_TOOL_*_v2_0.yml` 用 `yaml.safe_load` 取出 `fact_verification`/`market_claim_scan`/`returns_adapter`/`delivery_finalize` 四个节点的 `code` 字符串，`exec()` 后直接调用 `main()`，构造与既往任何测试用例都不同的测试文本（`fact_id: QA-TEST-FACT-777`/`QA-TEST-FACT-DOES-NOT-EXIST-999`、"当前最热的赛道就是这个方向"）。正向控制（合法 `fact_id`、无市场断言）与两组负向控制（不可解析 `fact_id`；注入市场断言短语）在 P1、P1.5 上均得到与设计一致的结果：`fact_gate_blocked`/`market_claim_blocked` 正确置位，`delivery_outcome` 分别为 `DELIVERED`/`NOT_DELIVERED_FACT_CHECK_BLOCKED`/`NOT_DELIVERED_MARKET_CLAIM_BLOCKED`，未出现"记一笔但照常交付"的情况。
- **两个已知坑核对**：坑一（P1 三种运行模式 `TOURNAMENT_ONLY`/`SELECTED_DIRECTION_TO_SCRIPT`/`FULL`）——`grep` 确认 `ALLOWED_RUN_MODES`/`run_mode_resolved`/「运行模式」判据表逐字保留，只删了表格"对应入口"列的 `ENTRY-04`/`ENTRY-05` 标签；坑二（P0 CTA 三级接缝）——`grep -c "CTA 三级接缝"` 命中 2 处（正文 1 + 历史改动日志提及 1），三级判据表与硬规则五条完整保留。两坑均未踩。
- **图结构完整性核验**：自建 `dsl_lib.py`（`check_graph_integrity`），对三份 `*_v2_0.yml` 做 `yaml.safe_load` 解析后逐节点核验：所有边的 `source`/`target` 指向存在节点；所有 `variables[].value_selector` 引用的上游节点/字段在该节点声明输出里能找到（`start`/`llm`/`template-transform` 按各自隐式输出规则特判）；所有 `end` 节点的 `outputs` 同样核验。三份文件均为 `nodes=16 edges=16`，无悬空引用、无断边。
- **T5 结转清单逐条处置**：CF-01/02/03 不适用于本任务（零模型调用，与 empirical/盲评/计分门运行期一致性无关）；CF-04 推迟到 S6（本轮未改动任何 references 加载/构建流程）；**CF-05** 维持 E1"推迟到共用层改造"的结论，同时如实记账 T3 新增的 `reversal_conditions[]` 客观加深了③"有立场但不固执"判据的承载，但明确这是 T3 任务本身的产物、不是专门为 CF-05 设计的承载机制，不因此声称已处理；**CF-06** 已处理（本轮层面）——本轮未触发拆分条件，`eval/` 顶层位置未受影响，完整禁带清单仍推迟到真正拆分时。详见 [sku-productization/CF_DISPOSITION_v1.0.md](../sku-productization/CF_DISPOSITION_v1.0.md)。
- **模型调用实际使用**：`0`（本任务全程不调 LLM；T2 四条接线要求全部以直接执行 Python 代码验证）。
- **原树与 E1 基线复算**：开工前与任务结束各复算一次，`content-production/` 全部 8 份被摘对象原始文件、`products/` 下 E1 摘出的六份基线文件 sha256 均与 E1 `EXTRACTION_MANIFEST_v1.0.json` 登记值一致，逐字节未变；`git status --short content-production/ account-operations/ decision-chain/` 输出为空，无任何 `M`（modified）标记。
- **终态**：`task_final_status = DONE`。不产生任何产品/商业结论、不评价三 SKU 质量。

- **ATT-002（E2-P 补丁，2026-09-02，同 `task_id` 续接，非新任务）**：Founder 对上述 ATT-001 交付独立复核（E2）通过后，另指出两处跨组件残留未清净，指示同会话开补丁收尾，不新开 `task_id`、不算 `REBASE`。
  - **P-1（历史改动日志表随系统提示词泄漏）**：核实 `resync_prompts.py` 的拼接规则——DSL `skill_llm` 系统角色 `prompt_template[0].text` = 对应 `SKILL_v2.0.md` 全文逐字节 + 固定附录，三份 `SKILL_v2.0.md` 里的"本版改了什么"/"v1.4 在 v1.3 基础上改了什么"两张历史改动日志表（P0 原第 21–57 行、P1 原第 24–38 行、P1.5 原第 16–28 行）因此**确实参与运行时喂入**（`grep` 事先确认三份 `_v2_0.yml` 均有命中：P0:1306、P1:730、P1.5:749）。既有判定"历史记录不得篡改"对**文档**继续有效，对**系统提示词**不成立——按 Founder 指示"搬走，不改写"：用 Python 按精确行号做整段搬移（非字符串重写），原文逐字迁入新文件 `sku-productization/E2P_RELOCATED_CHANGELOG_TABLES_v1.0.md`，随后重跑 `resync_prompts.py` 重新派生三份系统提示词。搬迁后复核：`grep` 三份 `SKILL_v2.0.md` + 三份 `_v2_0.yml` 均 0 命中该两张表标题；`check_graph_integrity` 三份复验 `nodes=16 edges=16`，与 ATT-001 收尾时一致（本次不改节点/边，只改字符串）。`REMOVAL_TRACE.md` 里原本记"不动"的三处对应行已改注为"文档判定不变，物理位置按 E2-P 搬走"。
  - **P-2（`return_id` 链核查）**：先查后动，未凭经验直接删。逐节点扫描三份 `_v2_0.yml` 的 `value_selector`/`outputs` 引用，确认 `professional_input_hash`（`envelope_check` 节点 `_sha(prof)` 计算）在全图**零读者**（三份文件一致），判定为纯死代码，删除其计算行与 `outputs` schema 声明（各文件两处）。同一批核查发现 `envelope_hash`（同节点 `_sha(env)` 计算）**有读者**——`component_return` 节点读取它构造 `return_id`（`"M4-RET-%s-%s" % (CAPABILITY, envelope_hash[:8])`，代码位置：P0 第 3009 行、P1 第 1823 行、P1.5 第 1883 行，均在 `component_return` 内，与本补丁指令原文提到的"`delivery_finalize` 中 `return_id` 的构造"有出入，以现场代码位置为准并已在报告里注明），`return_id` 连同六个同形字段被 `json.dumps` 成 `returns_json`，经 `end_component_return`（该分支的终态输出节点）暴露为 API 级输出字段之一。`component_return` 本身承担的"结构缺项→自然语言追问"功能此前已被 ATT-001 判定对齐表 §1.2 `MATCH`（非 `OUT_OF_CONTRACT`），但它内部这个与已删除的跨组件 Return 闭环同形的七字段对象是否该继续保留，本次核查够不成单方面判断——按指令"有人读→不删，停下来报告"处理，**未删除** `envelope_hash`/`return_id`/`component_return` 相关代码，作为第三个发现完整记入 `REMOVAL_TRACE.md` §四，转交 Founder 裁决，不在本补丁内自行定夺。
  - **验证**：三份 DSL 图结构复验（`nodes=16 edges=16`，两轮读数一致）；针对 `return_to_script`/`return_to_production`/`stale_set`/`binding_record`/`envelope_hash`/`professional_input_hash`/`system_prompt_sha256`/`task_contract_hash`/`ENTRY-07`/`DERIVE_MODE_AND_PACKAGE` 十项关键词在三份 `_v2_0.yml` + 三份 `SKILL_v2.0.md` 做 grep，除 `envelope_hash`（18 处，逐条核对均为 P-2 主动保留项）外全部 0 命中；E1 六份基线 `sha256sum` 现场复算与既有登记值逐字节一致；`content-production/`／`account-operations/`／`decision-chain/` 原树 `git status --short`/`git diff --stat` 均为空；`git status --short` 确认本补丁改动范围仅 6 份 `products/` 下 v2.0 文件 + 1 份新增文档，无越权改动。
  - **模型调用实际使用**：`0`（全部为确定性脚本/grep/hash 复算，延续 ATT-001 约束）。
  - **终态**：`e2p_final_status = P1_DONE / P2_PARTIAL_CLOSED_PENDING_FOUNDER_DECISION`。P-2 的"停下来报告"是本补丁指令要求的正确终态之一，不视为遗留缺陷；T-016 整体 `task_final_status` 维持 `DONE`，E2-P 未推翻 ATT-001 任何已验证结论。

...
### ATT-002（重排与桌面交付，`DONE`；同一 `task_id` 的收尾步，零模型调用）

- **输入与授权**：追加语句 `PP_盲评_重排与桌面交付_追加语句_v1.0.md`；母 Prompt sha256
  `d9b812f81f97e3203b94a9d0d0db75d8beda4a45539df34b3a1ead75c9fa9f03` 与勘误 001 sha256
  `e3f27d947f04e7173df7a1c759775abe30ffcb07f2da380b15777d763b7d498a` 现场复算一致，两份边界
  继续有效。Founder 2026-09-01「先重排再取件」＋「把仅需盲审的文档放到桌面」授权。新会话
  执行，未读取、未引用上一会话任何输出。
- **重排**：`pp-blind-review/sealed/FINAL_MAPPING_SEALED.json` 全部 15 项以全新随机种子
  （`random.SystemRandom`，独立于任何既往种子）重新执行：(a) 15 个逻辑条目整体重新随机
  排序（含 2 个同源对照条目的位置）；(b) 每个条目的甲／乙标签独立重新随机翻转。仓库内
  `pp-blind-review/delivery/01-15` 的 `任务描述.md`／`甲.md`／`乙.md` 已据此重新生成。
  **一致性自查**：脚本核验槽位集合 = 01–15 完整、`kind` 分布与重排前相同、每个
  `same_source` 条目的 `任务描述.md` 内容与其 `based_on_group` 对应的 `real` 条目逐字节
  一致——`CONSISTENCY_CHECK OK`，delivery/ 与新封存映射零差异。产出层
  （`freeze/`／`inputs/`／`runs/`／`sealed/EXTRA_SLOTS_RAW.json`／
  `sealed/SAME_SOURCE_CONTROL_ENTRIES.json`）未触碰，零模型调用。
- **两个程序问题（不涉及映射内容）**：
  - **Q1**：甲／乙分配是**逐条独立随机**，不是固定配比。依据：母 Prompt P0-5 原文
    「每组的甲／乙随机分配……同一来源不得固定在同一侧」，本身就是逐组独立随机的设计
    口径；本次重排同样对 15 个条目逐条独立掷币（`random() < 0.5`），不是对一个固定
    甲/乙配比列表做排列。**因此不是固定配比**——此前任何一次被打印的汇总计数，不能
    保证在本次重排后继续成立：甲／乙分布本身是每次独立重排都会变化的随机变量，不是
    常数，这一点如实登记，供后续如需评估该次打印的实际影响时参考。
  - **Q2**：**基于现有证据推断，非逐项直接确认**——上一次重排（发现打印之后那次）
    在本节 ATT-001 与 [L5 SE-044](L5_SIDE_EFFECTS.md) 的记录用词是「整份重新随机组装」
    「新旧排列无关联」，且封存映射文件在该次重排时被整份重写而非局部改动；母 Prompt
    P0-5「条目整体随机排序」定义本身对全部 15 个槽位一体适用，不单独区分真实条目与
    同源对照条目。据此推断该次重排**同时**重排了两个同源对照位的位置。但本仓库现有
    证据（该任务分支自始至终只有一个提交，见下方 Git 部分）里没有一条独立留痕专门
    把「同源对照位置」单列出来确认，此结论按可信度阶梯标记为**推断**，不是**已确认**。
- **执行侧需要披露的一处操作瑕疵**（按 A4 不淡化、不省略）：在核验重排后交付包结构
  是否完整时，执行侧用脚本按「`任务描述.md` 内容是否重复」分组统计，一次性把分组结果
  （各分组各含哪些槽位编号）打印进了本次会话的工具调用输出。该输出**未包含**任何真实
  来源信息（未出现 PP／对照哪一侧、未出现甲乙标签、未出现具体文案），但确实暴露了
  **当时那一版、即将被覆盖的旧映射**里"哪些槽位是同源对照条目"这一项**位置分布**，
  与本步绝对约束第 1 条字面冲突（该约束明文列出"任何位置分布"）。发现后立即停止同类
  打印，此后全部核验脚本一律只输出布尔结果与总数、不再输出任何槽位级明细；并将本节
  「重排」一段描述的整份重新随机排序作为直接补救——该动作使这段被打印的旧位置分布
  对新交付包完全失效（旧槽位与新槽位之间的对应关系已被完全打散，不构成对最终交付
  内容的有效先验知识）。因这条披露本身也进入了本次会话的工具调用输出，若要求
  "评审人没有任何理由读取本次会话记录"这一条完全不打折扣，评审人仍应按追加语句
  既定方式只取桌面文件夹、不读会话记录——本条披露不依赖"本会话记录足够干净"这一
  前提，而是把补救动作（整份重排）本身作为独立于会话记录的失效机制登记在案。
- **执行侧需要披露的第二处操作瑕疵**（按 A4 不淡化、不省略）：首次为本步变更执行
  `git commit` 时，未预先想到 git 默认按内容相似度做 rename 检测——由于 15 个交付位
  的文件内容逐字节未变、只是换了槽位路径，`git commit` 的提交摘要把"旧槽位路径 →
  新槽位路径"逐条配对显示（`rename ... => ...`），这段配对信息打印进了本次会话的
  工具调用输出，同样与本步绝对约束第 1 条字面冲突（暴露旧一版映射的完整位置分布，
  雖不含任何真实来源信息）。发现后立即处置：该次提交尚未推送，`git reset --soft`
  撤销（不涉及改写已发布历史）；尝试用 `git -c diff.renames=false commit` 抑制该
  配对显示，验证后发现提交摘要仍然输出配对（该配置对 `git commit` 的摘要路径未生效），
  遂再次撤销未推送的提交，改用 `git commit -q`（静默提交）成功使摘要不再输出到会话
  记录，随后才正式提交推送。**结构性限制，一并披露**：这不是可以靠某个 git 参数
  彻底消除的漏洞——只要交付内容逐字节不变、只改变其所在槽位，任何人直接执行
  `git diff 776fd1a <本步提交>`（跳过中间被撤销的提交，直接比较两个端点）仍能凭
  内容相似度重新算出"旧槽位 → 新槽位"的对应关系，这是"内容不变、只重排位置"这种
  交付机制本身的固有属性，`-q`／`--no-renames` 之类选项只能阻止它出现在**当次命令
  的默认输出**里，不能阻止**未来任何人**用 `git diff`/`git log -p`/`git show` 主动
  对这两个提交做端到端比较后重新算出。这与追加语句 §3 已预先提示的风险同类——
  "不要翻提交历史，被作废的旧版映射可能仍在历史里"——本次只是把它的技术根因坐实、
  说得更具体：现有的防线本来就是**程序性**的（评审人按约定只取桌面文件夹、不翻会话
  记录、不翻提交历史），不是技术上不可逆推，这一点建议如实告知 Founder，供其判断是否
  需要在评审人一侧追加更强的隔离（例如换一台不接触本仓库的设备读取桌面交付件）。
- **桌面交付**：`/mnt/c/Users/Administrator/Desktop/笛语盲评_内容发布包装/`，从重排后的
  `pp-blind-review/delivery/` 逐字节拷贝 15 个交付位（`任务描述.md`／`甲.md`／`乙.md`，
  共 45 个文件）＋ `评审说明与评审单.md`（逐字节复制，1 个文件）＋ 新建
  `评审单_空白.md`（评审说明里的评审单模板复制 15 份并预先编号 01–15，1 个文件），
  共 47 个文件。未拷贝 `freeze/`／`inputs/`／`runs/`／`sealed/`、素材构造规格、对照
  提示词、设计文档与增补、任何 README 或说明生成过程/方法的文件。桌面写入范围仅限
  该一个新建文件夹——全部写操作（`os.makedirs`／`shutil.copyfile`）目标路径均以该
  文件夹为前缀；写入前后各核验一次桌面根目录，除新增该文件夹外，既有其余 4 项
  （`FASHION_COMMERCIAL_VISUAL_CORE_MODULE_v1.0`／`SKILL商业化`／`管理`／`desktop.ini`）
  零读取、零修改、零移动、零删除。详见 [L5 SE-045](L5_SIDE_EFFECTS.md)。
- **泄漏自查**：机械扫描沿用 `DELIVERY_LEAK_SELF_CHECK_v1.0.json` 的标记清单（含对
  `status:` 类内部状态头纯文本／`**加粗**`／`` `反引号` `` 三种包裹形式的覆盖），对
  桌面交付件 47 个文件全量扫描：`total_hits = 0`。人工抽查跨槽位、跨甲乙两侧的 3 个
  文件逐字通读：零命中；另对全部 47 个文件做 `status` 一词的独立字面统计核验：
  出现次数 `0`。仓库内 `pp-blind-review/delivery/`（46 个文件，含评审说明）同一扫描：
  `total_hits = 0`。
- **Git**：任务分支 `task/pp-blind-review-material-generation-v1` 提交并推送
  （`776fd1a..f2941c6`）；`main` 未动（本地/远程均仍在 `01a42b0`）；未改写**已发布**
  历史、未 force push——两次 `git reset --soft` 均发生在对应提交推送**之前**，被撤销
  的两个本地提交从未到达 `origin`，不构成对已发布历史的改写。该任务分支推送前只有
  一个已发布提交（`776fd1a`），被作废重排的旧版排列从未单独进入过已发布的 Git 历史
  （本地与远程 reflog 核验一致），故本次提交信息无需专门注明"作废"——没有可指向的
  已发布历史条目；上面两处披露涉及的是本次提交推送过程中的操作瑕疵，不是"旧版残留"。
- **终态**：`task_final_status = DONE`（收尾步）。不产生任何质量结论，不改变
  `TD-M5-04`／`M5-AC-05`／`M5-AC-06`，不构成对 `Q-COMM-04` 的裁决，不新增任何评分／
  开封／合并 `main` 的授权。

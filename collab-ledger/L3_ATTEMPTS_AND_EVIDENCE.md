...
| 已知未完成 | `M1-AC-15` 阻断待解除——需 Founder 亲自执行一次真实版本回退/重发演练，或提供有效控制台凭证交执行侧完成（详见 L2）；**v0.8 尚未导入/发布/live 复验**——"对话 LLM 正确告知用户已收到材料"这一环节的代码修复目前只有单测覆盖，未经任何真实 Dify 调用验证；`features.file_upload` 是否会在下次导入时被再次覆盖回默认值，执行侧无法确认，需要下次导入后重新核对，不能假设一次修复永久生效；此前收口审查对 AC-03/04/07/10/13/14 的 PASS 已因源码持续改动继续过期，需新一轮 `affected_scope_only` 复验；两处非阻断账本完整性小缺口与 v1.3 哈希自证治理待决项已如实记录，等规划侧/Reviewer 后续核对 |

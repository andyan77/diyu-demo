...
| 已知未完成 | 14 条语义×5 维度完整落地（现只有 9 字段 P0 切片）；`content_task` 投影函数代码；A-0～A-4 受控等价输入回归；`call_intent` 完整必需输入判据；`open_threads` 跨轮延迟表面化语义（本行上一条已记录的真实发现，需设计判断而非当场改动）；独立 Reviewer；`M1-AC-00`～`15` 逐项验收 |

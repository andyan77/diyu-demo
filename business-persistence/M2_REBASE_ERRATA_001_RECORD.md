...
| R-09b | `diyu_app` 角色实际可以 `CONNECT` 到 `dify`/`dify_plugin`（表级 SELECT 正确拒绝，未读到真实数据）——此前 TDR 声称的"REVOKE ALL 阻止连接"在 CONNECT 层面不准确 | 尝试 `REVOKE CONNECT ... FROM PUBLIC/diyu_app` 被权限分类器拦截（触及非本任务独占的共享数据库），**未修复，如实披露**，见 §7 | 未提交（阻塞，见下） |
...
**R-09b（`diyu_app` 对 `dify`/`dify_plugin` 的 CONNECT 权限未被撤销）**：修复动作（`REVOKE CONNECT ON DATABASE dify/dify_plugin FROM PUBLIC/diyu_app`）被 Claude Code 权限分类器拦截，理由是该操作触及不属于本 task_id 独占沙箱的共享数据库。未强行绕过。**此项需要 Founder 或具备该权限的操作者手动执行，或明确授权后由执行侧重试**。在此之前，`M2-AC-13`（迁移/权限/隔离）标记为 `NOT_VERIFIED`（而非之前的 PASS）——见 `M2_ACCEPTANCE_EVIDENCE.md` 对应行。

## 8. 本轮终态

因 `M2-AC-13`（受 R-09b 阻塞）与 `M2-AC-16`（受 R-08 阻塞，见下）当前均非 CURRENT PASS，按 Prompt §8.1：
...
**不是** `AWAITING_FOUNDER_DIFY_ACCEPTANCE`——本轮 Rebase 发现的问题比预期更多，尚未满足 §8.2 的全部前提。

...
>
> **2026-08-25 补充（Rebase/Errata 001）**：又发现并修复了 4 项问题——并发创建内容版本
> 曾裸 500（此前误披露为"已知限制不修"，现已真修）、迁移回滚对真实数据会裸崩溃（现已
> 改成清晰报错）、旧 Demo 兼容之前误写"没做"实际现在已经做了、M2 的数据库账号原本还
> 连得上 Dify 自己的数据库（读不到任何真实数据，但连接本不该被允许）——你已就此项
> 明确授权，现已现场执行修复并验证生效（详见 `M2_REBASE_ERRATA_001_RECORD.md` §7）。
> 唯一仍未完成的一项：本轮最新代码未能在真实 Dify 候选画布上重新运行（本会话没有
> 可用的已认证 Dify 会话或 API Key，未猜测/重建凭据）——如果你想让这一环也拿到最新
> 证据，需要你提供一次有效会话/凭据，或者你自己按下方六步场景跑一遍并把结果告诉我。
> 逐条最新状态见 `M2_ACCEPTANCE_EVIDENCE.md`，当前技术侧还不能声称"完全收口"，下方
> 六步场景仍可正常测试，但请把这次实测理解为"功能验证"而不是"最终验收"。
...
- 旧 Demo 会话态兼容读取**已经做了**（`POST /workspaces/{id}/tasks/legacy-import`，5 槽任务
  状态快照 + 真实历史 Matrix/Campaign/Content Brief 生产产物均可显式导入，见
  `M2_ACCEPTANCE_EVIDENCE.md` M2-AC-14）；本六步场景本身没有专门演示这条接口，因为它是
  一次性导入动作，不是持续运营循环的一部分——需要单独验证的话可以另起一次针对
  `legacy-import` 端点的请求
- 没有市场观察、打法版本化的端到端演示——底层能力已实现（见独立审查报告），但本次六步
  场景没有覆盖，需要的话可以另起一次工作流运行验证
- **本次候选画布未针对本轮最新代码重新真实运行**（无可用的已认证 Dify 会话，未向你索要/
  重建密码）——下方六步场景的运行证据仍是上一轮代码版本留下的，不是当前最新 commit 的
  证据；这条链路本身没有变化（本轮新增的是导入/周期决定等新增能力，不影响这六步走的
  这条路径），但严格意义上你现在点「运行」看到的画布配置还没有对着最新代码重新验证过

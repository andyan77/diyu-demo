...
>
> **2026-08-25 补充（Rebase/Errata 001）**：又发现并修复了 3 项问题——并发创建内容版本
> 曾裸 500（此前误披露为"已知限制不修"，现已真修）、迁移回滚对真实数据会裸崩溃（现已
> 改成清晰报错）、旧 Demo 兼容之前误写"没做"实际现在已经做了。同时发现一项**未修复**
> 的问题：M2 的数据库账号目前还连得上 Dify 自己的数据库（读不到任何真实数据，但连接
> 本不该被允许）——修复动作需要你或有权限的人手动执行，详见
> `M2_REBASE_ERRATA_001_RECORD.md` §7。逐条最新状态见 `M2_ACCEPTANCE_EVIDENCE.md`，当前
> 技术侧还不能声称"完全收口"，下方六步场景仍可正常测试，但请把这次实测理解为"功能
> 验证"而不是"最终验收"。

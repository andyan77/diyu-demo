...
> 真实重跑了一次六步候选画布，全部通过——这一项现已完成，不再是缺口。**当前唯一未决
> 事项**：数据库迁移回滚的"失败可恢复"要求本身该怎么达成，需要你决定——要么我实现一套
> 具体的自动改键规则，要么我们把这条验收标准改写成更准确的"失败清晰可诊断，需要人工
> 介入"。逐条最新状态见 `M2_ACCEPTANCE_EVIDENCE.md`，当前技术侧还不能声称"完全收口"，
> 下方六步场景仍可正常测试，但请把这次实测理解为"功能验证"而不是"最终验收"。
...
- **数据库迁移回滚遇到某些真实数据状态时不能自动完成**——会清晰拒绝并说明冲突原因（不
  再是裸崩溃），但仍需要人工介入才能真正回滚，不是"全自动可逆"；这一项需要你决定后续
  怎么处理（见本文件顶部说明），不影响下方六步场景本身的正常运行

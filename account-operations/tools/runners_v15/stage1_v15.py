...
三个脚本原本都写进第 8 轮的证据目录（ep24 / ep25）。**那是历史证据，不能覆盖**，
所以这里把它们的 `OUT` 改道到 ep31，再跑。改的只有落盘位置，判据一个字没动。
"""
import io
import json
import os
...
OUT = os.path.join(WT, "account-operations/evidence/ep31-structural-and-ac16-v15")
...
def main():
    ...
    rep = {"what": "第 9 轮第 1 段：部署后非模型取证", "zero_model_calls": True,
           "out_dir": OUT, "steps": rows, "all_pass": all(r["ok"] for r in rows)}
    json.dump(rep, io.open(os.path.join(OUT, "STAGE1_V15.json"), "w", encoding="utf-8"),

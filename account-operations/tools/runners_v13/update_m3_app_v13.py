#!/usr/bin/env python3
"""载体 v1.2：把 v1.1 闸门实测出的缺陷 A/B/D/E 与链式投影缺陷一起修进候选 App。
...
def main():
    ...
    st, pub = c.call("POST", f"/console/api/apps/{app_id}/workflows/publish", {
        "marked_name": "m3-cand-v1.3-pos",   # Dify 限 20 字符
        "marked_comment": "M3 v1.3: structured standing positions + M2 a7b8101 compat",  # Dify 限 100 字符
    })
    ...
    assert st == 200
    with open(os.path.join(SCRATCH, "v13", "m3_app_draft_graph_v13.json"), "w", encoding="utf-8") as f:

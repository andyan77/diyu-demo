#!/usr/bin/env python3
"""EP-08 v1.1 preparation: re-bind the four arms to SKILL.md carrier-v1.4.

Writes a NEW file (`_arms_and_holdouts_v5.json`) in a NEW evidence dir.
...
OUT_DIR = os.path.join(WORKTREE, "account-operations/evidence/ep08-module-ab-v14")
...
def main():
    ...
    # ---- 三臂必须逐字节不变，这是 Founder 第 7 条「不降低 A 基线」的机械证明 ----
    prev = json.load(open(os.path.join(
        WORKTREE, "account-operations/evidence/ep08-module-ab-v13",
        "_arms_and_holdouts_v4.json"), encoding="utf-8"))["arms"]
    frozen_drift = []
    for k in ("A", "Aplus", "Bprime"):
        got = sha(arms[k]["system_prompt"])
        if got != prev[k]["system_prompt_sha256"]:
            frozen_drift.append((k, prev[k]["system_prompt_sha256"][:16], got[:16]))
    if frozen_drift:
        print("拒绝生成：本该一字未改的臂变了", frozen_drift, file=sys.stderr)
        raise SystemExit(2)
    assert sha(arms["B"]["system_prompt"]) != prev["B"]["system_prompt_sha256"], \
        "B 臂哈希与第 7 轮相同 —— 说明 SKILL.md 没换成 v1.4，重跑没有意义"
    doc["arms_unchanged_vs_v13"] = ["A", "Aplus", "Bprime"]
    doc["arms_rebound_to_v14"] = ["B"]
    doc["b_arm_prev_sha256"] = prev["B"]["system_prompt_sha256"]

    with open(os.path.join(OUT_DIR, "_arms_and_holdouts_v5.json"), "w", encoding="utf-8") as f:
    ...
    print("file sha256:", sha(read(os.path.join(OUT_DIR, "_arms_and_holdouts_v5.json"))))
